...
export class ParallelUIService {
     ...
     */
    mapToolForServer(toolName, args) {
        ...
        console.log(`[ParallelUI] Tool mapping: ${toolName} -> ${mappedName}, args: ${JSON.stringify(args)} -> ${JSON.stringify(mappedArgs)}`);
     ...
     */
    extractNodeIdFromResult(result) {
        try {
     ...
     */
    coerceArgumentTypes(toolName, args) {
        const result = { ...args };
    ...
    toRgbObject(value) {
        ...
        if (typeof value === "string") {
            ...
            if (/^[0-9a-fA-F]{6}$/.test(cleanHex)) {
                ...
                console.log(`[ParallelUI] Converted hex "${str}" to RGB: {r: ${r.toFixed(3)}, g: ${g.toFixed(3)}, b: ${b.toFixed(3)}}`);
                return { r, g, b };
            }
        }
     ...
     */
    async executeParallelTasks(tasks, onProgress, containerFrame) {
     ...
     */
    async executeTasksDirectMcp(tasks, onProgress, containerFrame) {
        const startTime = Date.now();
        const results = [];
        ...
        for (let i = 0; i < tasks.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.all(batch.map((task) => this.executeTaskDirectMcp(task, onProgress, containerFrame)));
    ...
    isConnectionError(errorMessage) {
        ...
        return (lowerError.includes("disconnect") ||
     ...
     */
    async handleConnectionError(errorMessage) {
        if (!this.mcpHub)
            return false;
     ...
     */
    async executeTaskDirectMcp(task, onProgress, containerFrame) {
        const startTime = Date.now();
        const nodeIds = [];
        try {
            ...
            const { mappedName: rectToolName, mappedArgs: rectMappedArgs } = this.mapToolForServer("create_rectangle", rectArgs);
            ...
            if (rectNodeId) {
                ...
                if (cornerRadius > 0) {
                    try {
                        ...
                        console.log(`[ParallelUI Direct] Setting corner radius ${cornerRadius} for rectangle ${rectNodeId} (server: ${this.activeFigmaServer})`);
                        ...
                        console.log(`[ParallelUI Direct] ✓ Corner radius ${cornerRadius} set successfully for rectangle ${rectNodeId}`);
                    }
                    catch (e) {
                        console.error(`[ParallelUI Direct] ✗ Failed to set corner radius ${cornerRadius} for ${rectNodeId}:`, e);
                    }
                }
                else {
            ...
            let textNodeId = this.extractNodeIdFromResult(textResult);
            if (textNodeId) {
            ...
            if (textNodeId) {
                ...
                const { mappedName: colorToolName, mappedArgs: colorMappedArgs } = this.mapToolForServer("set_text_color", colorArgs);
     ...
     */
    async safeMcpToolCall(toolName, args) {
        if (!this.mcpHub) {
     ...
     */
    async callAIForUITask(api, prompt, task, nodeIds, onProgress) {
            ...
            try {
                ...
                for await (const chunk of stream) {
                    ...
                    if (chunk.type === "text") {
                        ...
                        if (partialChunk.id)
                            existing.id = partialChunk.id;
                        if (partialChunk.name)
                            existing.name = partialChunk.name;
                        if (partialChunk.arguments)
                            existing.arguments += partialChunk.arguments;
                    }
                ...
                if (toolUseBlocks.length === 0) {
                    ...
                    if (iterations === 1) {
                        ...
                        throw new Error(`模型 "${this.apiConfiguration?.apiModelId}" 未生成任何工具調用。\n` +
                ...
                const toolResults = await Promise.all(toolUseBlocks.map(async (toolUse) => {
                    ...
                    if (positioningTools.includes(toolName)) {
                        console.log(`[ParallelUI] Task ${task.id} - ${toolName} at position: (${args.x}, ${args.y})`);
                    }
                    ...
                    try {
                        ...
                        const mcpResult = await this.mcpHub.callTool(this.activeFigmaServer, mappedName, mappedArgs);
                            ...
                            if (textContentBlock && textContentBlock.type === "text") {
                                ...
                                if (!createdNodeId) {
                                    ...
                                    if (withIdMatch) {
                                        ...
                                        console.log(`[ParallelUI] Extracted node ID from 'with ID' format: ${createdNodeId}`);
                                    }
                                }
                                if (!createdNodeId) {
                                    ...
                                    if (nodeIdMatch) {
                                        ...
                                        console.log(`[ParallelUI] Extracted node ID from id field: ${createdNodeId}`);
                                    }
                                }
                                if (!createdNodeId) {
                                    ...
                                    if (anyNodeIdMatch) {
                                        ...
                                        console.log(`[ParallelUI] Extracted node ID from text pattern: ${createdNodeId}`);
                                    }
                                }
                                if (!createdNodeId) {
                                    console.warn(`[ParallelUI] Could not extract node ID from response:`, text.substring(0, 200));
                                }
                            }
                        }
                        if (createdNodeId) {
                            ...
                            if (toolName === "create_rectangle") {
                                ...
                                if (cornerRadius > 0) {
                                    try {
                                        ...
                                        console.log(`[ParallelUI] Auto-setting corner radius ${cornerRadius} for rectangle ${createdNodeId}`);
                                        await this.mcpHub.callTool(this.activeFigmaServer, "set_corner_radius", {
                                        ...
                                        console.log(`[ParallelUI] ✓ Corner radius ${cornerRadius} set successfully for ${createdNodeId}`);
                                    }
                                    catch (e) {
        ...
        const schemas = {
            ...
            create_rectangle: {
                ...
                properties: {
                    ...
                    radius: {
                        type: "number",
                        description: "REQUIRED! Corner radius for rounded corners (8 for normal buttons, width/2 for circular)",
                    },
...
export function getParallelUIService() {
    return ParallelUIService.getInstance();
}
//# sourceMappingURL=ParallelUIService.js.map