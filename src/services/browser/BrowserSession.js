...
export class BrowserSession {
    ...
    async doAction(action) {
        if (!this.page) {
            throw new Error("Cannot perform browser action: no active browser session. The browser must be launched first using the 'launch' action before other browser actions can be performed.");
        }
        const logs = [];
     ...
     */
    async forceLinksToSameTab(page) {
        try {
            await page.evaluate(() => {
                try {
                    // Ensure we only install once per document
                    if (window.__ROO_FORCE_SAME_TAB__)
                        return;
                    window.__ROO_FORCE_SAME_TAB__ = true;
                    ...
                    window.open = function (url, target, features) {
                        try {
                            ...
                            try {
                                return originalOpen.apply(window, [url, "_self", features]);
                            }
                            catch { }
                        }
                        return null;
                    ...
                    document.addEventListener("click", (ev) => {
                        const el = ev.target?.closest?.('a[target="_blank"]');
                        if (el && el.href) {
                            ev.preventDefault();
                            try {
    ...
    async press(key) {
        return this.doAction(async (page) => {
            ...
            mainKey = (mapping[mainKey.toLowerCase()] ?? mainKey);
     ...
     */
    async saveScreenshot(filePath, cwd) {
        ...
        if (!fullPath.startsWith(normalizedCwd + path.sep) && fullPath !== normalizedCwd) {
            throw new Error(`Screenshot path "${filePath}" resolves to "${fullPath}" which is outside the workspace "${normalizedCwd}". ` +
        ...
        return this.doAction(async (page) => {
            ...
            await page.screenshot({
                path: fullPath,
                type: imageType,
                quality: imageType === "png"
                    ? undefined
                    : (this.context.globalState.get("screenshotQuality") ?? 75),
            });
        });
    }
    /**
     ...
     */
    async drawCursorIndicator(page, coordinate) {
        const [x, y] = coordinate.split(",").map(Number);
        try {
     ...
     */
    async removeCursorIndicator(page) {
        try {
            await page.evaluate(() => {
    ...
    getViewportSize() {
        ...
        if (this.page) {
            ...
            if (vp?.width)
                this.lastViewportWidth = vp.width;
            if (vp?.height)
                this.lastViewportHeight = vp.height;
        }
    ...
    async extractDOMStructure() {
        ...
        const elements = await page.evaluate(() => {
            ...
            return Array.from(els)
                ...
                return {
                    ...
                    text: text || el.placeholder || el.value || '',
        ...
        if (isCalculator) {
            ...
            output += '**STANDARD CALCULATOR LAYOUT (this is CORRECT):**\n';
            output += '```\n';
            output += 'Row 1: C    /   *   -  ← Clear + top operators\n';
            output += 'Row 2: 7    8   9   +  ← Numbers 7-8-9 + operator on RIGHT (CORRECT!)\n';
            output += 'Row 3: 4    5   6   -  ← Numbers 4-5-6 + operator on RIGHT (CORRECT!)\n';
            output += 'Row 4: 1    2   3   =  ← Numbers 1-2-3 + equals (CORRECT!)\n';
            output += 'Row 5: 0        .      ← Zero + decimal\n';
            output += '```\n\n';
            output += '⚠️ **IMPORTANT:** Operators (+,-,*,/) on the RIGHT side of number rows is CORRECT!\n\n';
            output += '**Check ONLY these items:**\n';
            output += '- [ ] Are numbers 7,8,9 in a row ABOVE 4,5,6?\n';
            output += '- [ ] Are numbers 4,5,6 in a row ABOVE 1,2,3?\n';
            output += '- [ ] Is 0 at the BOTTOM?\n';
            output += '- [ ] Numbers descend from 7-8-9 to 1-2-3 to 0?\n\n';
            output += '**❌ REJECT ONLY IF:**\n';
            output += '- Numbers are in WRONG ORDER (e.g., 1-2-3 above 7-8-9)\n';
            output += '- 0 is NOT at the bottom\n';
            output += '- Missing essential buttons\n\n';
            output += '**✅ APPROVE if number order is 7→4→1→0 from top to bottom!**\n\n';
        }
