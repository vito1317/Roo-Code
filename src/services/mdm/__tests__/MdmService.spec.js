...
vi.mock("../../../i18n", () => ({
    t: vi.fn((key) => {
        const translations = {
            "mdm.errors.cloud_auth_required": "Your organization requires Roo Code Cloud authentication. Please sign in to continue.",
            "mdm.errors.organization_mismatch": "You must be authenticated with your organization's Roo Code Cloud account.",
...
const mockGetClerkBaseUrl = getClerkBaseUrl;
describe("MdmService", () => {
    ...
    describe("compliance checking", () => {
        ...
        it("should be non-compliant when wrong organization", async () => {
            ...
            expect(compliance.compliant).toBe(false);
            if (!compliance.compliant) {
                expect(compliance.reason).toContain("You must be authenticated with your organization's Roo Code Cloud account");
            }
        });
...
//# sourceMappingURL=MdmService.spec.js.map