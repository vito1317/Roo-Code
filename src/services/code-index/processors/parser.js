...
export class CodeParser {
    ...
    _chunkTextByLines(lines, filePath, fileHash, chunkType, seenSegmentHashes, baseStartLine = 1) {
        ...
        const createSegmentBlock = (segment, originalLineNumber, startCharIndex) => {
            ...
            const segmentHash = createHash("sha256")
                .update(`${filePath}-${originalLineNumber}-${originalLineNumber}-${startCharIndex}-${segment.length}-${segmentPreview}`)
                .digest("hex");
            if (!seenSegmentHashes.has(segmentHash)) {
                seenSegmentHashes.add(segmentHash);
                chunks.push({
            ...
            if (currentChunkLength > 0 && currentChunkLength + lineLength > effectiveMaxChars) {
                ...
                if (currentChunkLength >= MIN_BLOCK_CHARS &&
                    ...
                    for (let k = i - 2; k >= chunkStartLineIndex; k--) {
                        ...
                        if (potentialChunkLength >= MIN_BLOCK_CHARS &&
    ...
    processMarkdownSection(lines, filePath, fileHash, type, seenSegmentHashes, startLine, identifier = null) {
        ...
        const needsChunking = content.length > MAX_BLOCK_CHARS * MAX_CHARS_TOLERANCE_FACTOR ||
    ...
    parseMarkdownContent(filePath, content, fileHash, seenSegmentHashes) {
        ...
        if (markdownCaptures.length > 0) {
            ...
            if (firstHeaderLine > 0) {
                ...
                const preHeaderBlocks = this.processMarkdownSection(preHeaderLines, filePath, fileHash, "markdown_content", seenSegmentHashes, 1);
        ...
        for (let i = 0; i < markdownCaptures.length; i += 2) {
            ...
            const sectionBlocks = this.processMarkdownSection(sectionLines, filePath, fileHash, `markdown_header_h${headerLevel}`, seenSegmentHashes, startLine, headerText);
        ...
        if (lastProcessedLine < lines.length) {
            ...
            const remainingBlocks = this.processMarkdownSection(remainingLines, filePath, fileHash, "markdown_content", seenSegmentHashes, lastProcessedLine + 1);
