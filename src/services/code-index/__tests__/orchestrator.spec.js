...
describe("CodeIndexOrchestrator - error path cleanup gating", () => {
    ...
    beforeEach(() => {
        ...
        stateManager = {
            ...
            setSystemState: vi.fn().mockImplementation((state, _msg) => {
                currentState = state;
            }),
    ...
    it("should not call clearCollection() or clear cache when initialize() fails (indexing not started)", async () => {
        ...
        const orchestrator = new CodeIndexOrchestrator(configManager, stateManager, workspacePath, cacheManager, vectorStore, scanner, fileWatcher);
        // Act
