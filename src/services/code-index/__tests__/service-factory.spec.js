...
const MockedOpenAiEmbedder = OpenAiEmbedder;
const MockedCodeIndexOllamaEmbedder = CodeIndexOllamaEmbedder;
const MockedOpenAICompatibleEmbedder = OpenAICompatibleEmbedder;
const MockedGeminiEmbedder = GeminiEmbedder;
const MockedQdrantVectorStore = QdrantVectorStore;
// Import the mocked functions
import { getDefaultModelId, getModelDimension } from "../../../shared/embeddingModels";
const mockGetDefaultModelId = getDefaultModelId;
const mockGetModelDimension = getModelDimension;
describe("CodeIndexServiceFactory", () => {
    ...
    describe("createEmbedder", () => {
        ...
        it("should pass model ID to OpenAI Compatible embedder when using OpenAI Compatible provider", () => {
            ...
            factory.createEmbedder();
            // Assert
            expect(MockedOpenAICompatibleEmbedder).toHaveBeenCalledWith("https://api.example.com/v1", "test-api-key", testModelId);
        });
        it("should handle undefined model ID for OpenAI Compatible embedder", () => {
            ...
            factory.createEmbedder();
            // Assert
            expect(MockedOpenAICompatibleEmbedder).toHaveBeenCalledWith("https://api.example.com/v1", "test-api-key", undefined);
        });
    ...
    describe("createVectorStore", () => {
        ...
        it("should use config.modelId for OpenAI provider", () => {
            ...
            expect(MockedQdrantVectorStore).toHaveBeenCalledWith("/test/workspace", "http://localhost:6333", 3072, "test-key");
        });
        it("should use config.modelId for Ollama provider", () => {
            ...
            expect(MockedQdrantVectorStore).toHaveBeenCalledWith("/test/workspace", "http://localhost:6333", 768, "test-key");
        });
        ...
        it("should prioritize getModelDimension over manual modelDimension for OpenAI Compatible provider", () => {
            ...
            // Assert
            expect(mockGetModelDimension).toHaveBeenCalledWith("openai-compatible", testModelId);
            expect(MockedQdrantVectorStore).toHaveBeenCalledWith("/test/workspace", "http://localhost:6333", modelDimension, // Should use model's built-in dimension, not manual
            "test-key");
        });
        it("should use manual modelDimension only when model has no built-in dimension", () => {
            ...
            // Assert
            expect(mockGetModelDimension).toHaveBeenCalledWith("openai-compatible", testModelId);
            expect(MockedQdrantVectorStore).toHaveBeenCalledWith("/test/workspace", "http://localhost:6333", manualDimension, // Should use manual dimension as fallback
            "test-key");
        });
        ...
        it("should throw error when manual modelDimension is invalid for OpenAI Compatible", () => {
            ...
            // Act & Assert
            expect(() => factory.createVectorStore()).toThrow("serviceFactory.vectorDimensionNotDeterminedOpenAiCompatible");
        });
        ...
        it("should use default model when config.modelId is undefined", () => {
            ...
            expect(MockedQdrantVectorStore).toHaveBeenCalledWith("/test/workspace", "http://localhost:6333", 1536, "test-key");
        });
...
//# sourceMappingURL=service-factory.spec.js.map