...
vi.mock("../../../../i18n", () => ({
    t: (key, params) => {
        const translations = {
            ...
            "embeddings:authenticationFailed": "Failed to create embeddings: Authentication failed. Please check your OpenRouter API key.",
...
describe("OpenRouterEmbedder", () => {
    ...
    describe("createEmbeddings", () => {
        ...
        it("should not include provider routing when specificProvider is default", async () => {
            const embedderWithDefaultProvider = new OpenRouterEmbedder(mockApiKey, undefined, undefined, OPENROUTER_DEFAULT_PROVIDER_NAME);
            const testEmbedding = new Float32Array([0.25, 0.5]);
            const base64String = Buffer.from(testEmbedding.buffer).toString("base64");
            const mockResponse = {
                data: [
