...
vitest.mock("../../../../i18n", () => ({
    t: (key, params) => {
        const translations = {
            "embeddings:authenticationFailed": "Failed to create embeddings: Authentication failed. Please check your AWS credentials.",
...
describe("BedrockEmbedder", () => {
    ...
    beforeEach(() => {
        ...
        const MockedBedrockRuntimeClient = BedrockRuntimeClient;
    ...
    describe("createEmbeddings", () => {
        ...
        it("should create embeddings with Nova multimodal model", async () => {
            const novaMultimodalEmbedder = new BedrockEmbedder("us-east-1", "test-profile", "amazon.nova-2-multimodal-embeddings-v1:0");
            const testTexts = ["Hello world"];
            const mockResponse = {
                body: new TextEncoder().encode(JSON.stringify({
                    embeddings: [
        ...
        it("should handle Nova multimodal model with multiple texts", async () => {
            ...
            const firstBodyStr = typeof firstCommand.input.body === "string"
            ...
            const secondBodyStr = typeof secondCommand.input.body === "string"
        ...
        describe("retry logic", () => {
            ...
            it("should not retry on non-throttling errors", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: Unauthorized");
        ...
        describe("error handling", () => {
            it("should handle API errors gracefully", async () => {
                ...
                expect(console.error).toHaveBeenCalledWith(expect.stringContaining("Bedrock embedder error"), expect.any(Error));
            });
            ...
            it("should handle AWS-specific errors", async () => {
                ...
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: Invalid credentials");
                // Test AccessDeniedException
                const accessError = new Error("Access denied");
                accessError.name = "AccessDeniedException";
                mockSend.mockRejectedValueOnce(accessError);
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: Access denied");
                // Test ResourceNotFoundException
                const notFoundError = new Error("Model not found");
                notFoundError.name = "ResourceNotFoundException";
                mockSend.mockRejectedValueOnce(notFoundError);
                await expect(embedder.createEmbeddings(testTexts)).rejects.toThrow("Failed to create embeddings after 3 attempts: Model not found");
            });
        });
    });
