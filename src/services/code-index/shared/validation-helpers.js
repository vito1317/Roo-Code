...
export function sanitizeErrorMessage(errorMessage) {
    ...
    sanitized = sanitized.replace(/(?:https?|ftp|file):\/\/(?:localhost|[\w\-\.]+)(?::\d+)?(?:\/[\w\-\.\/\?\&\=\#]*)?/gi, "[REDACTED_URL]");
    ...
    sanitized = sanitized.replace(/(?:\/[\w\-\.]+)+(?:\/[\w\-\.\s]*)*|(?:[A-Za-z]:\\[\w\-\.\\]+)|(?:\.{1,2}\/[\w\-\.\/]+)/g, "[REDACTED_PATH]");
 ...
 */
export function getErrorMessageForStatus(status, embedderType) {
    switch (status) {
        case 401:
        case 403:
...
export function extractStatusCode(error) {
    ...
    if (serialized?.response?.status)
        return serialized.response.status;
    return undefined;
}
/**
 ...
 */
export function handleValidationError(error, embedderType, customHandlers) {
 ...
 */
export async function withValidationErrorHandling(validationFn, embedderType, customHandlers) {
    try {
 ...
 */
export function formatEmbeddingError(error, maxRetries) {
