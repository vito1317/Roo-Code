...
export class SimpleInstaller {
    ...
    async installItem(item, options) {
    ...
    async installMode(item, target) {
        ...
        if (this.customModesManager) {
            ...
            try {
                ...
                if (modeData?.slug) {
                    const slugLineIndex = lines.findIndex((l) => l.includes(`slug: ${modeData.slug}`) || l.includes(`slug: "${modeData.slug}"`));
                    if (slugLineIndex >= 0) {
        ...
        try {
            ...
            if (error.code === "ENOENT") {
                ...
                throw new Error(`Cannot install mode: The ${fileName} file contains invalid YAML. ` +
        ...
        existingData.customModes = existingData.customModes.filter((mode) => mode.slug !== modeData.slug);
        // Add the new mode
        ...
        if (addedModeIndex >= 0) {
            ...
            if (addedMode?.slug) {
                const slugLineIndex = lines.findIndex((l) => l.includes(`slug: ${addedMode.slug}`) || l.includes(`slug: "${addedMode.slug}"`));
                if (slugLineIndex >= 0) {
                    line = slugLineIndex + 1; // Convert to 1-based line number
                }
            }
    ...
    async installMcp(item, target, options) {
        if (!item.content) {
        ...
        if (options?.parameters?._selectedIndex !== undefined && Array.isArray(item.content)) {
            ...
            if (index >= 0 && index < item.content.length) {
                ...
                const uniqueParametersForNewMethod = Array.from(new Map(allParametersForNewMethod.map((p) => [p.key, p])).values());
        ...
        try {
            ...
            if (error.code === "ENOENT") {
                ...
                throw new Error(`Cannot install MCP server: The ${fileName} file contains invalid JSON. ` +
...
//# sourceMappingURL=SimpleInstaller.js.map