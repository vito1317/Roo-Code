...
const initWorkspaceRepo = async ({ workspaceDir, userName = "Roo Code", userEmail = "support@roocode.com", testFileName = "test.txt", textFileContent = "Hello, world!", }) => {
...
describe.each([[RepoPerTaskCheckpointService, "RepoPerTaskCheckpointService"]])("CheckpointService", (klass, prefix) => {
    ...
    describe(`${klass.name}#hasNestedGitRepositories`, () => {
        it("throws error when nested git repositories are detected during initialization", async () => {
            ...
            await expect(service.initShadowGit()).rejects.toThrowError(/Checkpoints are disabled because a nested git repository was detected at:/);
            // Clean up.
            vitest.restoreAllMocks();
