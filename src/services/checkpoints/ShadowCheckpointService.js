...
function createSanitizedGit(baseDir) {
    ...
    if (removedVars.length > 0) {
        console.log(`[createSanitizedGit] Removed git environment variables for checkpoint isolation: ${removedVars.join(", ")}`);
    }
    const options = {
        baseDir,
        config: [],
...
export class ShadowCheckpointService extends EventEmitter {
    ...
    async initShadowGit(onInit) {
        ...
        if (nestedGitPath) {
            ...
            throw new Error(`Checkpoints are disabled because a nested git repository was detected at: ${relativePath}. ` +
    ...
    async getNestedGitRepository() {
        try {
            ...
            const nestedGitPaths = gitPaths.filter(({ type, path: filePath }) => {
                ...
                return (normalizedPath.includes(".git/HEAD") &&
            ...
            if (nestedGitPaths.length > 0) {
                ...
                this.log(`[${this.constructor.name}#getNestedGitRepository] found ${nestedGitPaths.length} nested git repositories, first at: ${repoDir}`);
                return absolutePath;
            }
            return null;
        }
        catch (error) {
            this.log(`[${this.constructor.name}#getNestedGitRepository] failed to check for nested git repos: ${error instanceof Error ? error.message : String(error)}`);
