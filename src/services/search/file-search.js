...
export async function executeRipgrepForFiles(workspacePath, limit) {
    // Get limit from configuration if not provided
    const effectiveLimit = limit ?? vscode.workspace.getConfiguration(Package.name).get("maximumIndexedFilesForFileSearch", 10000);
    const args = [
        "--files",
        "--follow",
        "--hidden",
