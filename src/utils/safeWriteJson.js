...
async function safeWriteJson(filePath, data, options) {
    ...
    try {
        ...
        actualTempNewFilePath = path.join(path.dirname(absoluteFilePath), `.${path.basename(absoluteFilePath)}.new_${Date.now()}_${Math.random().toString(36).substring(2)}.tmp`);
        ...
        try {
            ...
            actualTempBackupFilePath = path.join(path.dirname(absoluteFilePath), `.${path.basename(absoluteFilePath)}.bak_${Date.now()}_${Math.random().toString(36).substring(2)}.tmp`);
        ...
        if (actualTempBackupFilePath) {
            try {
                ...
                console.error(`Successfully wrote ${absoluteFilePath}, but failed to clean up backup ${actualTempBackupFilePath}:`, unlinkBackupError);
            }
        }
    }
    catch (originalError) {
        ...
        if (backupFileToRollbackOrCleanupWithinCatch) {
            try {
                ...
                console.error(`[Catch] Failed to restore backup ${backupFileToRollbackOrCleanupWithinCatch} to ${absoluteFilePath}:`, rollbackError);
            }
        }
        ...
        if (newFileToCleanupWithinCatch) {
            try {
                ...
                console.error(`[Catch] Failed to clean up temporary new file ${newFileToCleanupWithinCatch}:`, cleanupError);
            }
        }
        ...
        if (actualTempBackupFilePath) {
            try {
                ...
                console.error(`[Catch] Failed to clean up temporary backup file ${actualTempBackupFilePath}:`, cleanupError);
            }
        }
...
async function _streamDataToFile(targetPath, data, prettyPrint = false) {
    ...
    const stringifyStream = new JsonStreamStringify(data === undefined ? null : data, undefined, // replacer
...
//# sourceMappingURL=safeWriteJson.js.map