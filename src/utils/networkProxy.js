 ...
 */
export async function initializeNetworkProxy(context, channel) {
    ...
    log(`Proxy config: enabled=${config.enabled}, serverUrl=${redactProxyUrl(config.serverUrl)}, tlsInsecure=${config.tlsInsecure}`);
    ...
    if (typeof vscode.workspace.onDidChangeConfiguration === "function") {
        context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration(`${Package.name}.debugProxy.enabled`) ||
...
async function configureGlobalProxy(config) {
    ...
    try {
        ...
        log(`Failed to load global-agent (proxy support is only available in debug/dev builds): ${error instanceof Error ? error.message : String(error)}`);
        return;
    }
