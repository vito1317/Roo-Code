 ...
 * ```
 */
export const ToolInputSchema = z.lazy(() => z
    .object({
 ...
 */
const NormalizedToolSchemaInternal = z.lazy(() => z
    .object({
    ...
    .passthrough()
    .transform((schema) => {
    const { type, required, properties, additionalProperties, format, items, minItems, maxItems, uniqueItems, ...rest } = schema;
    const result = { ...rest };
    // Determine if this schema represents an object type
    const isObjectType = type === "object" || (Array.isArray(type) && type.includes("object")) || properties !== undefined;
...
function flattenTopLevelComposition(schema) {
    ...
    const objectVariant = compositionArray.find((variant) => typeof variant === "object" &&
        variant !== null &&
