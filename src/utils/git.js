 ...
 */
export async function getGitRepositoryInfo(workspaceRoot) {
    try {
 ...
 */
export function convertGitUrlToHttps(url) {
    try {
 ...
 */
export function extractRepositoryName(url) {
    try {
 ...
 */
export async function getGitStatus(cwd, maxFiles = 20) {
    try {
        const isInstalled = await checkGitInstalled();
        if (!isInstalled) {
            return null;
