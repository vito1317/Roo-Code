...
describe("injectEnv", () => {
    ...
    it("should use notFoundValue for missing env variables", async () => {
        ...
        expect(result).toBe(expectedString);
        expect(consoleWarnSpy).toHaveBeenCalledWith(`[injectVariables] variable "MISSING_VAR" referenced but not found in "env"`);
    ...
    it("should use default empty string for missing env variables if notFoundValue is not provided", async () => {
        ...
        const result = await injectEnv(configString);
        expect(result).toBe(expectedString);
        expect(consoleWarnSpy).toHaveBeenCalledWith(`[injectVariables] variable "ANOTHER_MISSING" referenced but not found in "env"`);
        consoleWarnSpy.mockRestore();
    });
