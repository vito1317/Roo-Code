...
export class ChutesHandler extends RouterProvider {
    constructor(options) {
        super({
            options,
            name: "chutes",
    ...
    getCompletionParams(systemPrompt, messages, metadata) {
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        if (model.id.includes("DeepSeek-R1")) {
            ...
            const stream = await this.client.chat.completions.create(this.getCompletionParams(systemPrompt, messages, metadata));
            for await (const chunk of stream) {
                const delta = chunk.choices[0]?.delta;
                if (delta?.content) {
