...
import { anthropicDefaultModelId, anthropicModels, ANTHROPIC_DEFAULT_MAX_TOKENS, ApiProviderError, } from "@roo-code/types";
...
export class AnthropicHandler extends BaseProvider {
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        let { id: modelId, betas = ["fine-grained-tool-streaming-2025-05-14"], maxTokens, temperature, reasoning: thinking, } = this.getModel();
        ...
        switch (modelId) {
            ...
            case "claude-3-haiku-20240307": {
                 ...
                 * current request.
                 */
                const userMsgIndices = sanitizedMessages.reduce((acc, msg, index) => (msg.role === "user" ? [...acc, index] : acc), []);
                ...
                try {
                    ...
                    TelemetryService.instance.captureException(new ApiProviderError(error instanceof Error ? error.message : String(error), this.providerName, modelId, "createMessage"));
                    throw error;
                }
                break;
            }
        ...
        if (inputTokens > 0 || outputTokens > 0 || cacheWriteTokens > 0 || cacheReadTokens > 0) {
            const { totalCost } = calculateApiCostAnthropic(this.getModel().info, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens);
            yield {
                type: "usage",
                inputTokens: 0,
                outputTokens: 0,
    ...
    async completePrompt(prompt) {
        ...
        try {
            ...
            TelemetryService.instance.captureException(new ApiProviderError(error instanceof Error ? error.message : String(error), this.providerName, model, "completePrompt"));
            throw error;
        }
