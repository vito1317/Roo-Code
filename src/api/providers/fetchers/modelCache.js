...
export const getModels = async (options) => {
    ...
    try {
        ...
        if (modelCount > 0) {
            ...
            await writeModels(provider, models).catch((err) => console.error(`[MODEL_CACHE] Error writing ${provider} models to file cache:`, err));
        }
        else {
 ...
 */
export const refreshModels = async (options) => {
    const { provider } = options;
    ...
    const refreshPromise = (async () => {
        try {
            ...
            await writeModels(provider, models).catch((err) => console.error(`[refreshModels] Error writing ${provider} models to disk:`, err));
            return models;
        }
        catch (error) {
 ...
 */
export const flushModels = async (options, refresh = false) => {
    const { provider } = options;
    if (refresh) {
