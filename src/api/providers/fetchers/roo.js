 ...
 */
export async function getRooModels(baseUrl, apiKey) {
    ...
    try {
        ...
        try {
            ...
            for (const model of parsed.data.data) {
                ...
                const apiSettings = model.settings;
                const apiVersionedSettings = model.versionedSettings;
                // Start with base model info
                let modelInfo = { ...baseModelInfo };
                ...
                if (apiVersionedSettings) {
                    const resolvedVersionedSettings = resolveVersionedSettings(apiVersionedSettings);
                    if (Object.keys(resolvedVersionedSettings).length > 0) {
                        ...
                        modelInfo = { ...modelInfo, ...apiSettings };
                    }
                }
                else if (apiSettings) {
