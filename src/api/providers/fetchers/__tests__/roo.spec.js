...
describe("getRooModels", () => {
    ...
    it("should fetch and parse models successfully", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith("https://api.roocode.com/proxy/v1/models", expect.objectContaining({
    ...
    it("should handle HTTP errors", async () => {
        ...
        await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow("Failed to fetch Roo Code Cloud models: HTTP 401: Unauthorized");
    });
    it("should handle timeout", async () => {
        const abortError = new Error("AbortError");
        abortError.name = "AbortError";
        mockFetch.mockRejectedValueOnce(abortError);
        await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow("Failed to fetch Roo Code Cloud models: Request timed out");
    });
    it("should handle invalid response format", async () => {
        ...
        await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow("Failed to fetch Roo Code Cloud models: Unexpected response format");
    });
    ...
    it("should handle network errors", async () => {
        ...
        await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow("Failed to fetch Roo Code Cloud models: No response from server");
    });
    ...
    it("should handle arbitrary settings properties dynamically", async () => {
        ...
        const models = await getRooModels(baseUrl, apiKey);
        const model = models["test/dynamic-settings-model"];
    ...
    it("should handle model with only versionedSettings and no plain settings", async () => {
        ...
        const models = await getRooModels(baseUrl, apiKey);
        const model = models["test/versioned-only-model"];
    ...
    it("should select highest matching version from versionedSettings", async () => {
        ...
        const models = await getRooModels(baseUrl, apiKey);
        const model = models["test/multi-version-model"];
    ...
    it("should apply highest versionedSettings for nightly builds (package name)", async () => {
        ...
        Package.name = "roo-code-nightly";
        try {
            const models = await getRooModels(baseUrl, apiKey);
            const model = models["test/nightly-version-model"];
