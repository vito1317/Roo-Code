...
import { z } from "zod";
import { ApiProviderError, openRouterDefaultModelId, openRouterDefaultModelInfo, OPENROUTER_DEFAULT_PROVIDER_NAME, OPEN_ROUTER_PROMPT_CACHING_MODELS, DEEP_SEEK_DEFAULT_TEMPERATURE, } from "@roo-code/types";
import { TelemetryService } from "@roo-code/telemetry";
import { NativeToolCallParser } from "../../core/assistant-message/NativeToolCallParser";
import { convertToOpenAiMessages, sanitizeGeminiMessages, consolidateReasoningDetails, } from "../transform/openai-format";
...
import { generateImageWithProvider } from "./utils/image-generation";
...
export class OpenRouterHandler extends BaseProvider {
    ...
    handleStreamingError(error, modelId, operation) {
        ...
        const apiError = Object.assign(new ApiProviderError(rawErrorMessage, this.providerName, modelId, operation, error?.code), { status: error?.code, error });
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        let openAiMessages = [
            ...
            ...convertToOpenAiMessages(messages, isMistral ? { normalizeToolCallId: normalizeMistralToolCallId } : undefined),
        ];
        ...
        try {
            ...
            if (parseResult.success && parseResult.data.error) {
                ...
                const apiError = Object.assign(new ApiProviderError(rawErrorMessage, this.providerName, modelId, "createMessage", openRouterError.error?.code), {
    ...
    async completePrompt(prompt) {
        ...
        try {
            ...
            if (parseResult.success && parseResult.data.error) {
                ...
                const parsedError = extractErrorFromMetadataRaw(rawString);
                const rawErrorMessage = parsedError || openRouterError.error?.message || "Unknown error";
                const apiError = Object.assign(new ApiProviderError(rawErrorMessage, this.providerName, modelId, "completePrompt", openRouterError.error?.code), {
                    status: openRouterError.error?.code,
                    error: openRouterError.error,
                });
                TelemetryService.instance.captureException(apiError);
