...
import { vertexDefaultModelId, vertexModels, ANTHROPIC_DEFAULT_MAX_TOKENS, VERTEX_1M_CONTEXT_MODEL_IDS, } from "@roo-code/types";
...
export class AnthropicVertexHandler extends BaseProvider {
    options;
    client;
    constructor(options) {
        super();
    ...
    getModel() {
        ...
        const supports1MContext = VERTEX_1M_CONTEXT_MODEL_IDS.includes(id);
    ...
    async completePrompt(prompt) {
        try {
            let { id, info: { supportsPromptCache }, temperature, maxTokens = ANTHROPIC_DEFAULT_MAX_TOKENS, reasoning: thinking, } = this.getModel();
            const params = {
                model: id,
