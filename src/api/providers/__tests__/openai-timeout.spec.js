...
describe("OpenAiHandler timeout configuration", () => {
    ...
    it("should use custom timeout for OpenAI-compatible providers", () => {
        ;
        getApiRequestTimeout.mockReturnValue(1800000); // 30 minutes
        const options = {
    ...
    it("should use timeout for Azure OpenAI", () => {
        ;
        getApiRequestTimeout.mockReturnValue(900000); // 15 minutes
        const options = {
            apiModelId: "gpt-4",
            openAiModelId: "gpt-4",
        ...
        new OpenAiHandler(options);
        expect(mockAzureOpenAIConstructor).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=openai-timeout.spec.js.map