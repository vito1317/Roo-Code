...
describe("AwsBedrockHandler", () => {
    ...
    describe("region mapping and cross-region inference", () => {
        ...
        describe("AWS GovCloud and China partition support", () => {
            it("should parse AWS GovCloud ARNs (arn:aws-us-gov:bedrock:...)", () => {
                ...
                const parseArn = handler.parseArn.bind(handler);
                const result = parseArn("arn:aws-us-gov:bedrock:us-gov-west-1:123456789012:inference-profile/us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0");
                expect(result.isValid).toBe(true);
            ...
            it("should parse AWS China ARNs (arn:aws-cn:bedrock:...)", () => {
                ...
                const parseArn = handler.parseArn.bind(handler);
                const result = parseArn("arn:aws-cn:bedrock:cn-north-1:123456789012:inference-profile/anthropic.claude-3-sonnet-20240229-v1:0");
                expect(result.isValid).toBe(true);
            ...
            it("should accept GovCloud custom ARN in handler constructor", () => {
                const handler = new AwsBedrockHandler({
                    ...
                    awsSecretKey: "test-secret-key",
                    awsRegion: "us-gov-west-1",
                    awsCustomArn: "arn:aws-us-gov:bedrock:us-gov-west-1:123456789012:inference-profile/us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0",
                });
                // Should not throw and should return valid model info
                const modelInfo = handler.getModel();
                expect(modelInfo.id).toBe("arn:aws-us-gov:bedrock:us-gov-west-1:123456789012:inference-profile/us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0");
                expect(modelInfo.info).toBeDefined();
            });
            it("should accept China region custom ARN in handler constructor", () => {
                const handler = new AwsBedrockHandler({
                    ...
                    awsSecretKey: "test-secret-key",
                    awsRegion: "cn-north-1",
                    awsCustomArn: "arn:aws-cn:bedrock:cn-north-1:123456789012:inference-profile/anthropic.claude-3-sonnet-20240229-v1:0",
                });
                // Should not throw and should return valid model info
                const modelInfo = handler.getModel();
                expect(modelInfo.id).toBe("arn:aws-cn:bedrock:cn-north-1:123456789012:inference-profile/anthropic.claude-3-sonnet-20240229-v1:0");
                expect(modelInfo.info).toBeDefined();
            });
            it("should detect region mismatch in GovCloud ARN", () => {
                ...
                const result = parseArn("arn:aws-us-gov:bedrock:us-gov-west-1:123456789012:inference-profile/us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0", "us-east-1");
                expect(result.isValid).toBe(true);
                expect(result.region).toBe("us-gov-west-1");
    ...
    describe("1M context beta feature", () => {
        ...
        it("should include anthropic_beta parameter when 1M context is enabled", async () => {
            ...
            expect(commandArg.additionalModelRequestFields.anthropic_beta).toContain("fine-grained-tool-streaming-2025-05-14");
    ...
    describe("service tier feature", () => {
        ...
        describe("pricing multipliers in getModel()", () => {
            it("should apply FLEX tier pricing with 50% discount", () => {
                ...
                const model = handler.getModel();
                const baseModel = bedrockModels[supportedModelId];
            ...
            it("should not apply service tier pricing for unsupported models", () => {
                ...
                const model = handler.getModel();
                const baseModel = bedrockModels[unsupportedModelId];
    ...
    describe("error telemetry", () => {
        ...
        it("should capture telemetry on createMessage error", async () => {
            ...
            const clientInstance = vi.mocked(BedrockRuntimeClient).mock.results[vi.mocked(BedrockRuntimeClient).mock.results.length - 1]
                ?.value;
            const mockSendFn = clientInstance?.send;
