...
class TestOpenAiCompatibleProvider extends BaseOpenAiCompatibleProvider {
    constructor(apiKey) {
        const testModels = {
            "test-model": {
                maxTokens: 4096,
...
describe("BaseOpenAiCompatibleProvider Timeout Configuration", () => {
    ...
    it("should pass the default timeout to the OpenAI client constructor", () => {
        ;
        getApiRequestTimeout.mockReturnValue(600000); // 600 seconds in ms
        new TestOpenAiCompatibleProvider("test-api-key");
    ...
    it("should use custom timeout value from getApiRequestTimeout", () => {
        ;
        getApiRequestTimeout.mockReturnValue(1800000); // 30 minutes in ms
        new TestOpenAiCompatibleProvider("test-api-key");
        expect(mockOpenAIConstructor).toHaveBeenCalledWith(expect.objectContaining({
            timeout: 1800000,
        }));
...
//# sourceMappingURL=base-openai-compatible-provider-timeout.spec.js.map