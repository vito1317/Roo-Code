...
describe("AwsBedrockHandler Native Tool Calling", () => {
    ...
    describe("createMessage with native tools", () => {
        it("should include toolConfig when tools are provided", async () => {
            ...
            const generator = handlerWithNativeTools.createMessage("You are a helpful assistant.", [{ role: "user", content: "Read the file at /test.txt" }], metadata);
        ...
        it("should always include fine-grained tool streaming beta for Claude models", async () => {
            ...
            const generator = handlerWithNativeTools.createMessage("You are a helpful assistant.", [{ role: "user", content: "Hello" }], metadata);
            await generator.next();
            expect(mockConverseStreamCommand).toHaveBeenCalled();
            const commandArg = mockConverseStreamCommand.mock.calls[0][0];
...
//# sourceMappingURL=bedrock-native-tools.spec.js.map