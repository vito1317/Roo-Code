...
describe("NativeOllamaHandler", () => {
    ...
    describe("tool calling", () => {
        it("should include tools when tools are provided", async () => {
            ...
            const stream = handler.createMessage("System", [{ role: "user", content: "What's the weather?" }], { taskId: "test", tools });
            // Consume the stream
            for await (const _ of stream) {
                // consume stream
            }
        ...
        it("should not include tools when no tools are provided", async () => {
            ...
            expect(mockChat).toHaveBeenCalledWith(expect.not.objectContaining({
        ...
        it("should yield tool_call_partial when model returns tool calls", async () => {
            ...
            const stream = handler.createMessage("System", [{ role: "user", content: "What's the weather in SF?" }], { taskId: "test", tools });
            const results = [];
            for await (const chunk of stream) {
                results.push(chunk);
            }
        ...
        it("should yield tool_call_end events after tool_call_partial chunks", async () => {
            ...
            const stream = handler.createMessage("System", [{ role: "user", content: "What's the weather and time in SF?" }], { taskId: "test", tools });
            const results = [];
            for await (const chunk of stream) {
                results.push(chunk);
            }
