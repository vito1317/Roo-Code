...
describe("Bedrock ARN Handling", () => {
    ...
    describe("parseArn direct tests", () => {
        ...
        it("should set crossRegionInference to true when a known region prefix is found in the model ID", () => {
            const handler = createHandler();
            const arn = "arn:aws:bedrock:us-east-1:123456789012:foundation-model/us.anthropic.claude-3-sonnet-20240229-v1:0";
            // Access the private method using type casting
            const result = handler.parseArn(arn, "us-east-1");
        ...
        it("should set crossRegionInference to true for model IDs with apac (4 digit) region prefix", () => {
            ...
            const arn = "arn:aws:bedrock:ap-east-1:123456789012:foundation-model/apac.anthropic.claude-3-sonnet-20240229-v1:0";
            // Access the private method using type casting
            const result = handler.parseArn(arn, "us-east-1");
            // Verify crossRegionInference is true
            expect(result.crossRegionInference).toBe(true);
    ...
    describe("ARN handling in constructor and methods", () => {
        ...
        it("should extract region from ARN and use it for client configuration", () => {
            ...
            const handler = createHandler({
                awsRegion: "us-east-1",
                awsCustomArn: "arn:aws:bedrock:eu-west-1:123456789012:inference-profile/anthropic.claude-3-sonnet-20240229-v1:0",
            });
        ...
        it("should log region mismatch warning when ARN region differs from provided region", () => {
            ...
            const arn = "arn:aws:bedrock:eu-west-1:123456789012:inference-profile/anthropic.claude-3-sonnet-20240229-v1:0";
            createHandler({
                awsCustomArn: arn,
                awsRegion: "us-east-1", // Different from ARN region
            });
            // Verify logger was called with region mismatch warning
            expect(infoSpy).toHaveBeenCalledWith(expect.stringContaining("Region mismatch"), expect.objectContaining({
...
//# sourceMappingURL=bedrock-custom-arn.spec.js.map