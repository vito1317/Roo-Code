...
import { bedrockDefaultModelId, bedrockModels, bedrockDefaultPromptRouterModelId, BEDROCK_DEFAULT_TEMPERATURE, BEDROCK_MAX_TOKENS, BEDROCK_DEFAULT_CONTEXT, AWS_INFERENCE_PROFILE_MAPPING, BEDROCK_1M_CONTEXT_MODEL_IDS, BEDROCK_GLOBAL_INFERENCE_MODEL_IDS, BEDROCK_SERVICE_TIER_MODEL_IDS, BEDROCK_SERVICE_TIER_PRICING, ApiProviderError, } from "@roo-code/types";
...
export class AwsBedrockHandler extends BaseProvider {
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        const is1MContextEnabled = BEDROCK_1M_CONTEXT_MODEL_IDS.includes(baseModelId) && this.options.awsBedrock1MContext;
        // Determine if service tier should be applied (checked later when building payload)
        const useServiceTier = this.options.awsBedrockServiceTier && BEDROCK_SERVICE_TIER_MODEL_IDS.includes(baseModelId);
        if (useServiceTier) {
    ...
    getModel() {
        ...
        if (this.options.awsCustomArn) {
            ...
            if (this.options.awsUseGlobalInference &&
                BEDROCK_GLOBAL_INFERENCE_MODEL_IDS.includes(baseIdForGlobal)) {
        ...
        // Use parseBaseModelId to handle cross-region inference prefixes
        const baseModelId = this.parseBaseModelId(modelConfig.id);
        if (BEDROCK_1M_CONTEXT_MODEL_IDS.includes(baseModelId) && this.options.awsBedrock1MContext) {
        ...
        if (this.options.awsBedrockServiceTier && BEDROCK_SERVICE_TIER_MODEL_IDS.includes(baseModelIdForTier)) {
        ...
        return tools
            ...
            toolSpec: {
                ...
                inputSchema: {
                    ...
                    json: normalizeToolSchema(tool.function.parameters),
                },
            },
        }));
    }
