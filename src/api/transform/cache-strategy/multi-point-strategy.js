...
export class MultiPointStrategy extends CacheStrategy {
    ...
    determineOptimalCachePoints() {
        ...
        const useSystemCache = supportsSystemCache && this.config.systemPrompt && this.meetsMinTokenThreshold(this.systemTokenCount);
    ...
    determineMessageCachePoints(minTokensPerPoint, remainingCachePoints) {
        ...
        if (previousPlacements.length === 0) {
            ...
            while (currentIndex < totalMessages && remainingCachePoints > 0) {
                const newPlacement = this.findOptimalPlacementForRange(currentIndex, totalMessages - 1, minTokensPerPoint);
                if (newPlacement) {
        ...
        if (newMessagesTokens >= minTokensPerPoint) {
            ...
            if (remainingCachePoints > previousPlacements.length) {
                ...
                const newPlacement = this.findOptimalPlacementForRange(lastPreviousIndex + 1, totalMessages - 1, minTokensPerPoint);
                if (newPlacement) {
                    placements.push(newPlacement);
                }
            }
                ...
                if (newMessagesTokens >= requiredTokenThreshold) {
                    ...
                    for (let i = 0; i < previousPlacements.length; i++) {
                        if (i !== smallestGapIndex && i !== smallestGapIndex + 1) {
                            ...
                            const combinedPlacement = this.findOptimalPlacementForRange(startOfRange, combinedEndIndex, minTokensPerPoint);
    ...
    formatWithoutCachePoints() {
        const systemBlocks = this.config.systemPrompt
            ? [{ text: this.config.systemPrompt }]
            : [];
        return this.formatResult(systemBlocks, this.messagesToContentBlocks(this.config.messages));
    }
}
//# sourceMappingURL=multi-point-strategy.js.map