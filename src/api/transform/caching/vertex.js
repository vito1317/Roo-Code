export function addCacheBreakpoints(messages) {
    ...
    return messages.map((message, index) => message.role !== "assistant" && (index === lastIndex || index === secondLastIndex)
...
function cachedMessage(message) {
    ...
    const lastTextBlockIndex = message.content.reduce((lastIndex, content, index) => (content.type === "text" ? index : lastIndex), -1);
    // Then use this pre-calculated index in the map function.
    return {
        ...message,
        content: message.content.map((content, index) => content.type === "text"
            ? {
                ...content,
