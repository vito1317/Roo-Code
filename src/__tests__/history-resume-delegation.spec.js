...
describe("History resume delegation - parent metadata transitions", () => {
    ...
    it("reopenParentFromDelegation persists parent metadata (delegated → active) before reopen", async () => {
        ...
        expect(updateTaskHistory).toHaveBeenCalledWith(expect.objectContaining({
            id: "parent-1",
            status: "active",
        ...
        expect(createTaskWithHistoryItem).toHaveBeenCalledWith(expect.objectContaining({
            status: "active",
            completedByChildId: "child-1",
        }), { startTask: false });
    });
    it("reopenParentFromDelegation injects subtask_result into both UI and API histories", async () => {
        ...
        expect(saveTaskMessages).toHaveBeenCalledWith(expect.objectContaining({
        ...
        expect(saveApiMessages).toHaveBeenCalledWith(expect.objectContaining({
            messages: expect.arrayContaining([
                expect.objectContaining({
                    role: "user",
    ...
    it("reopenParentFromDelegation injects tool_result when new_task tool_use exists in API history", async () => {
        ...
        expect(injectedMsg.content[0].type).toBe("tool_result");
        expect(injectedMsg.content[0].tool_use_id).toBe("toolu_abc123");
    });
    it("reopenParentFromDelegation injects plain text when no new_task tool_use exists in API history", async () => {
        ...
        expect(injected.content[0].text).toContain("Subtask c-no-tool completed");
    });
    it("reopenParentFromDelegation sets skipPrevResponseIdOnce via resumeAfterDelegation", async () => {
        const parentInstance = {
            skipPrevResponseIdOnce: false,
            resumeAfterDelegation: vi.fn().mockImplementation(async function () {
    ...
    it("handles empty history gracefully when injecting synthetic messages", async () => {
        ...
        vi.mocked(readTaskMessages).mockResolvedValue([]);
        vi.mocked(readApiMessages).mockResolvedValue([]);
        await expect(ClineProvider.prototype.reopenParentFromDelegation.call(provider, {
            parentTaskId: "p5",
            childTaskId: "c5",
...
//# sourceMappingURL=history-resume-delegation.spec.js.map