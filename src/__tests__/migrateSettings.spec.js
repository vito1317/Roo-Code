...
describe("Settings Migration", () => {
    ...
    it("should migrate custom modes file if old file exists and new file doesn't", async () => {
        ...
        vitest.mocked(fileExistsAtPath).mockImplementation(async (path) => {
            if (path === mockSettingsDir)
                return true;
            if (path === legacyClineCustomModesPath)
                return true;
    ...
    it("should migrate MCP settings file if old file exists and new file doesn't", async () => {
        ...
        vitest.mocked(fileExistsAtPath).mockImplementation(async (path) => {
            if (path === mockSettingsDir)
                return true;
            if (path === legacyMcpSettingsPath)
                return true;
            if (path === legacyClineCustomModesPath)
                return false; // Ensure this file doesn't exist
    ...
    it("should handle errors gracefully", async () => {
        ...
        expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining("Error migrating settings files"));
    });
    it("should convert custom_modes.json to YAML format", async () => {
        ...
        expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining("original JSON file preserved for rollback purposes"));
    });
    it("should handle corrupt JSON gracefully", async () => {
        ...
        await migrateSettings(mockContext, mockOutputChannel);
        // Verify error was logged
        expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining("Error parsing custom_modes.json"));
    ...
    it("should skip migration when YAML file already exists", async () => {
        ...
        vitest.mocked(fileExistsAtPath).mockImplementation(async (path) => {
            ...
            if (path === legacyCustomModesJson)
                return true;
            if (path === newCustomModesYaml)
                return true; // YAML already exists
            if (path === legacyClineCustomModesPath)
                return false;
            if (path === legacyMcpSettingsPath)
                return false;
        ...
        expect(mockOutputChannel.appendLine).toHaveBeenCalledWith("custom_modes.yaml already exists, skipping migration");
        // Verify no file operations occurred
        expect(mockWrite).not.toHaveBeenCalled();
        expect(mockUnlink).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=migrateSettings.spec.js.map