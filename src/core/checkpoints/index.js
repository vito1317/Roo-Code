...
export async function getCheckpointService(task, { interval = 250 } = {}) {
...
async function checkGitInstallation(task, service, log, provider) {
    try {
        ...
        if (!gitInstalled) {
            ...
            const selection = await vscode.window.showWarningMessage(t("common:errors.git_not_installed"), t("common:buttons.learn_more"));
...
export async function checkpointSave(task, force = false, suppressMessage = false) {
...
export async function checkpointRestore(task, { ts, commitHash, mode, operation = "delete" }) {
    const service = await getCheckpointService(task);
    if (!service) {
        return;
    }
    ...
    try {
        ...
        if (mode === "restore") {
            ...
            const { totalTokensIn, totalTokensOut, totalCacheWrites, totalCacheReads, totalCost } = getApiMetrics(task.combineMessages(deletedMessages));
...
export async function checkpointDiff(task, { ts, previousCommitHash, commitHash, mode }) {
    const service = await getCheckpointService(task);
    if (!service) {
        return;
    }
