...
export async function getEnvironmentDetails(cline, includeFileDetails = false) {
    let details = "";
    const clineProvider = cline.providerRef.deref();
    const state = await clineProvider?.getState();
    const { terminalOutputLineLimit = 500, terminalOutputCharacterLimit = DEFAULT_TERMINAL_OUTPUT_CHARACTER_LIMIT, maxWorkspaceFiles = 200, } = state ?? {};
    ...
    if (busyTerminals.length > 0) {
        ...
        for (const busyTerminal of busyTerminals) {
            ...
            if (newOutput) {
                newOutput = Terminal.compressTerminalOutput(newOutput, terminalOutputLineLimit, terminalOutputCharacterLimit);
                terminalDetails += `\n### New Output\n${newOutput}`;
            }
        }
    }
    ...
    if (terminalsWithOutput.length > 0) {
        ...
        for (const inactiveTerminal of terminalsWithOutput) {
            ...
            for (const process of completedProcesses) {
                ...
                if (output) {
                    output = Terminal.compressTerminalOutput(output, terminalOutputLineLimit, terminalOutputCharacterLimit);
    ...
    if (isBrowserActive) {
        ...
        const configuredViewport = state?.browserViewportSize ?? "900x600";
        let configuredWidth;
    ...
    if (includeFileDetails) {
        ...
        if (isDesktop) {
            ...
            if (maxFiles === 0) {
                ...
                const result = formatResponse.formatFilesList(cline.cwd, files, didHitLimit, cline.rooIgnoreController, showRooIgnoredFiles);
                details += result;
            }
        }
    }
