 ...
 */
async function askArchitectForApproval(cline, toolDescription) {
...
請決定是否允許執行此工具。回答 "APPROVE" 或 "REJECT: [原因]"`,
        ...
        for await (const chunk of stream) {
            if (chunk.type === "text") {
                ...
                await cline.say("text", header + responseText, undefined, true);
                if (isFirstChunk) {
 ...
 */
export async function presentAssistantMessage(cline) {
    if (cline.abort) {
    ...
    try {
        ...
        console.error(`Block content:`, JSON.stringify(cline.assistantMessageContent[cline.currentStreamingContentIndex], null, 2));
        cline.presentAssistantMessageLocked = false;
        return;
    }
    switch (block.type) {
        case "mcp_tool_use": {
            ...
            const askApproval = async (type, partialMessage, progressStatus, isProtected) => {
                ...
                if (shouldRouteToArchitect && type === "tool") {
                    ...
                    const { approved, feedback } = await askArchitectForApproval(cline, partialMessage || toolDescription());
                    if (!approved) {
                        if (feedback) {
        ...
        case "text": {
            ...
            if (content) {
                ...
                if (containsXmlToolMarkup(content)) {
                    const errorMessage = "XML tool calls are no longer supported. Remove any XML tool markup (e.g. <read_file>...</read_file>) and use native tool calling instead.";
        ...
        case "tool_use": {
            ...
            if (!toolCallId) {
                const errorMessage = "Invalid tool call: missing tool_use.id. XML tool calls are no longer supported. Remove any XML tool markup (e.g. <read_file>...</read_file>) and use native tool calling instead.";
                // Record a tool error for visibility/telemetry. Use the reported tool name if present.
                try {
                    if (typeof cline.recordToolError === "function" &&
            ...
            if (!block.partial) {
                ...
                if (isKnownTool && !block.nativeArgs && !customTool) {
                    const errorMessage = `Invalid tool call for '${block.name}': missing nativeArgs. ` +
            ...
            try {
                ...
                const hasStarted = messages.some((m) => m.say === "browser_action_result");
                let isClosed = false;
            ...
            if (!block.partial) {
                ...
                if (!repetitionCheck.allowExecution && repetitionCheck.askUser) {
                    ...
                    TelemetryService.instance.captureException(new ConsecutiveMistakeError(`Tool repetition limit reached for ${block.name}`, cline.taskId, cline.consecutiveMistakeCount, cline.consecutiveMistakeLimit, "tool_repetition", cline.apiConfiguration.apiProvider, cline.api.getModel().id));
            ...
            switch (block.name) {
                case "write_to_file":
                    ...
                    break;
                case "update_todo_list":
                    await updateTodoListTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "search_replace":
                    await checkpointSaveAndMark(cline);
                    await searchReplaceTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "edit_file":
                    await checkpointSaveAndMark(cline);
                    await editFileTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "apply_patch":
                    await checkpointSaveAndMark(cline);
                    await applyPatchTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "read_file":
                    ...
                    break;
                case "fetch_instructions":
                    await fetchInstructionsTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "list_files":
                    await listFilesTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "codebase_search":
                    await codebaseSearchTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "search_files":
                    await searchFilesTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "browser_action":
                    await browserActionTool(cline, block, askApproval, handleError, pushToolResult);
                    break;
                case "execute_command":
                    ...
                    break;
                case "use_mcp_tool":
                    await useMcpToolTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "access_mcp_resource":
                    await accessMcpResourceTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "ask_followup_question":
                    await askFollowupQuestionTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "switch_mode":
                    await switchModeTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "new_task":
                    await newTaskTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                        toolCallId: block.id,
                ...
                case "attempt_completion": {
                    ...
                    await attemptCompletionTool.handle(cline, block, completionCallbacks);
                    break;
                }
                case "run_slash_command":
                    await runSlashCommandTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "generate_image":
                    await checkpointSaveAndMark(cline);
                    await generateImageTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "handoff_context":
                    await handoffContextTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "start_background_service":
                    await startBackgroundServiceTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "parallel_ui_tasks":
                    await parallelUITasksTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                ...
                case "parallel_mcp_calls":
                    await parallelMcpCallsTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
                    break;
                case "adjust_layout":
                    await adjustLayoutTool.handle(cline, block, {
                        askApproval,
                        handleError,
                        pushToolResult,
                    });
...
function containsXmlToolMarkup(text) {
