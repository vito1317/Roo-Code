...
vi.mock("../../tools/validateToolUse", () => ({
    ...
    isValidToolName: vi.fn((toolName) => ["read_file", "write_to_file", "ask_followup_question", "attempt_completion", "use_mcp_tool"].includes(toolName)),
}));
...
describe("presentAssistantMessage - Image Handling in Native Tool Calling", () => {
    ...
    beforeEach(() => {
        ...
        mockTask.pushToolResultToUserContent = vi.fn().mockImplementation((toolResult) => {
            const existingResult = mockTask.userMessageContent.find((block) => block.type === "tool_result" && block.tool_use_id === toolResult.tool_use_id);
            if (existingResult) {
                return false;
            }
    ...
    it("should preserve images in tool_result for native tool calling", async () => {
        ...
        const toolResult = mockTask.userMessageContent.find((item) => item.type === "tool_result" && item.tool_use_id === toolCallId);
    ...
    it("should fail fast when tool_use is missing id (legacy/XML-style tool call)", async () => {
        ...
        expect(textBlocks.some((b) => String(b.text).includes("XML tool calls are no longer supported"))).toBe(true);
        // Should not proceed to execute tool or add images as tool output.
        expect(mockTask.userMessageContent.some((item) => item.type === "image")).toBe(false);
    });
    ...
    describe("Multiple tool calls handling", () => {
        it("should send tool_result with is_error for skipped tools in native tool calling when didRejectTool is true", async () => {
            ...
            const toolResult = mockTask.userMessageContent.find((item) => item.type === "tool_result" && item.tool_use_id === toolCallId2);
            ...
            const textBlocks = mockTask.userMessageContent.filter((item) => item.type === "text" && item.text.includes("due to user rejecting"));
        ...
        it("should send tool_result with is_error for skipped tools in native tool calling when didAlreadyUseTool is true", async () => {
            ...
            // Ensure no text blocks were added for this rejection
            const textBlocks = mockTask.userMessageContent.filter((item) => item.type === "text" && item.text.includes("was not executed because"));
            expect(textBlocks.length).toBe(0);
        });
        it("should reject subsequent tool calls when a legacy/XML-style tool call is encountered", async () => {
            ...
            expect(mockTask.userMessageContent.some((item) => item.type === "tool_result")).toBe(false);
        });
...
//# sourceMappingURL=presentAssistantMessage-images.spec.js.map