 ...
 */
const ALIAS_TO_CANONICAL = new Map(Object.entries(TOOL_ALIASES).map(([alias, canonical]) => [alias, canonical]));
/**
 ...
 */
export function applyToolAliases(allowedTools) {
 ...
 */
export function getToolAliasGroup(toolName) {
    return ALIAS_GROUPS.get(toolName) ?? [toolName];
}
export function applyModelToolCustomization(allowedTools, modeConfig, modelInfo) {
    if (!modelInfo) {
    ...
    if (modelInfo.includedTools && modelInfo.includedTools.length > 0) {
        ...
        const allowedGroups = new Set(modeConfig.groups.map((groupEntry) => (Array.isArray(groupEntry) ? groupEntry[0] : groupEntry)));
 ...
 */
export function filterNativeToolsForMode(nativeTools, mode, customModes, experiments, codeIndexManager, settings, mcpHub) {
    ...
    let allowedToolNames = new Set(allToolsForMode.filter((tool) => isToolAllowedForMode(tool, modeSlug, customModes ?? [], undefined, undefined, experiments ?? {})));
    // Apply model-specific tool customization
    const modelInfo = settings?.modelInfo;
    const { allowedTools: customizedTools, aliasRenames } = applyModelToolCustomization(allowedToolNames, modeConfig, modelInfo);
 ...
 */
export function isToolAllowedInMode(toolName, mode, customModes, experiments, codeIndexManager, settings) {
    const modeSlug = mode ?? defaultModeSlug;
    ...
    return isToolAllowedForMode(canonicalTool, modeSlug, customModes ?? [], undefined, undefined, experiments ?? {});
}
/**
 ...
 */
export function getAvailableToolsInGroup(groupName, mode, customModes, experiments, codeIndexManager, settings) {
 ...
 */
export function filterMcpToolsForMode(mcpTools, mode, customModes, experiments) {
    const modeSlug = mode ?? defaultModeSlug;
    // MCP tools are always in the mcp group, check if use_mcp_tool is allowed
    const isMcpAllowed = isToolAllowedForMode("use_mcp_tool", modeSlug, customModes ?? [], undefined, undefined, experiments ?? {});
