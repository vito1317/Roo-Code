...
describe("getMcpServerTools", () => {
    ...
    it("should return empty array when no servers are available", () => {
        ...
        const result = getMcpServerTools(mockHub);
        expect(result).toEqual([]);
    });
    ...
    it("should deduplicate tools when same server exists in both global and project configs", () => {
        const globalServer = createMockServer("context7", [createMockTool("resolve-library-id", "Global description")], "global");
        const projectServer = createMockServer("context7", [createMockTool("resolve-library-id", "Project description")], "project");
...
//# sourceMappingURL=mcp_server.spec.js.map