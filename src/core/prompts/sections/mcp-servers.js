export async function getMcpServersSection(mcpHub, diffStrategy, enableMcpServerCreation, includeToolDescriptions = true) {
    if (!mcpHub) {
        return "";
    }
    const connectedServers = mcpHub.getServers().length > 0
		...
		${JSON.stringify(tool.inputSchema, null, 2).split("\n").join("\n    ")}`
            ...
            return (`## ${server.name}${config.command ? ` (\`${config.command}${config.args && Array.isArray(config.args) ? ` ${config.args.join(" ")}` : ""}\`)` : ""}` +
                (server.instructions ? `\n\n### Instructions\n${server.instructions}` : "") +
                (tools ? `\n\n### Available Tools\n${tools}` : "") +
                (templates ? `\n\n### Resource Templates\n${templates}` : "") +
                (resources ? `\n\n### Direct Resources\n${resources}` : ""));
