...
import { LOCK_TEXT_SYMBOL } from "../ignore/RooIgnoreController";
export const formatResponse = {
    toolDenied: () => JSON.stringify({
        status: "denied",
        message: "The user denied this operation.",
    }),
    toolDeniedWithFeedback: (feedback) => JSON.stringify({
        status: "denied",
        feedback,
    }),
    toolApprovedWithFeedback: (feedback) => JSON.stringify({
        status: "approved",
        feedback,
    }),
    toolError: (error) => JSON.stringify({
        status: "error",
        message: "The tool execution failed",
        error,
    }),
    rooIgnoreError: (path) => JSON.stringify({
        status: "error",
        type: "access_denied",
...
(This is an automated message, so do not respond to it conversationally.)`;
    ...
    tooManyMistakes: (feedback) => JSON.stringify({
        status: "guidance",
        feedback,
    }),
    ...
    invalidMcpToolArgumentError: (serverName, toolName) => JSON.stringify({
        status: "error",
    ...
    unknownMcpToolError: (serverName, toolName, availableTools) => JSON.stringify({
        status: "error",
        type: "unknown_tool",
    ...
    unknownMcpServerError: (serverName, availableServers) => JSON.stringify({
        status: "error",
        type: "unknown_server",
    ...
    formatFilesList: (absolutePath, files, didHitLimit, rooIgnoreController, showRooIgnoredFiles, rooProtectedController) => {
        const sorted = files
            .map((file) => {
