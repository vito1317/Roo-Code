...
export class MessageManager {
    ...
    async truncateApiHistoryWithCleanup(cutoffTs, removedIds, skipCleanup) {
        ...
        if (!hasExactMatch && hasMessageBeforeCutoff) {
            ...
            const firstUserMsgIndexToRemove = apiHistory.findIndex((m) => m.ts !== undefined && m.ts >= cutoffTs && m.role === "user");
        ...
        const historyChanged = apiHistory.length !== originalHistory.length || apiHistory.some((msg, i) => msg !== originalHistory[i]);
        if (historyChanged) {
