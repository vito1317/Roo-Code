    ...
    describe("getEnvironmentDetails", () => {
        describe("API conversation handling", () => {
            ...
            describe("processUserContentMentions", () => {
                it("should process mentions in user_message tags", async () => {
                    ...
                    expect(processedContent[1].text).toContain("<user_message>Text with 'some/path' (see below for file content) in user_message tags</user_message>");
                    ...
                    expect(content1.text).toContain("<user_message>Check 'some/path' (see below for file content)</user_message>");
        ...
        describe("Subtask Rate Limiting", () => {
            ...
            it("should enforce rate limiting across parent and subtask", async () => {
                ...
                expect(saySpy).toHaveBeenCalledWith("api_req_rate_limit_wait", expect.stringMatching(/\{"seconds":\d+\}/), undefined, true);
    ...
    describe("abortTask", () => {
        ...
        describe("Stream Failure Retry", () => {
            it("should not abort task on stream failure, only on user cancellation", async () => {
                ...
                console.error(`[Task#${task.taskId}.${task.instanceId}] Stream failed, will retry: ${streamFailureError.message}`);
...
describe("Queued message processing after condense", () => {
    function createProvider() {
        ...
        const provider = new ClineProvider(ctx, output, "sidebar", new ContextProxy(ctx));
...
describe("pushToolResultToUserContent", () => {
    ...
    it("should prevent duplicate tool_result with same tool_use_id", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Skipping duplicate tool_result for tool_use_id: duplicate-id"));
        warnSpy.mockRestore();
    });
    ...
    it("should not interfere with other content types in userMessageContent", () => {
        ...
        task.userMessageContent.push({ type: "text", text: "Some text" }, { type: "image", source: { type: "base64", media_type: "image/png", data: "base64data" } });
        const toolResult = {
            type: "tool_result",
            tool_use_id: "test-id",
            content: "Result",
