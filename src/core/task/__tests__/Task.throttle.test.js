...
describe("Task token usage throttling", () => {
    ...
    test("should emit TaskTokenUsageUpdated immediately on first change", async () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith(RooCodeEventName.TaskTokenUsageUpdated, task.taskId, expect.any(Object), expect.any(Object));
    });
    test("should throttle subsequent emissions within 2 seconds", async () => {
        ...
        const firstEmitCount = emitSpy.mock.calls.filter((call) => call[0] === RooCodeEventName.TaskTokenUsageUpdated).length;
        ...
        const secondEmitCount = emitSpy.mock.calls.filter((call) => call[0] === RooCodeEventName.TaskTokenUsageUpdated).length;
        ...
        const thirdEmitCount = emitSpy.mock.calls.filter((call) => call[0] === RooCodeEventName.TaskTokenUsageUpdated).length;
    ...
    test("should include toolUsage in emission payload", async () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith(RooCodeEventName.TaskTokenUsageUpdated, task.taskId, expect.any(Object), // tokenUsage
        task.toolUsage);
    });
    ...
    test("emitFinalTokenUsageUpdate should emit on tool usage change alone", async () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith(RooCodeEventName.TaskTokenUsageUpdated, task.taskId, expect.any(Object), task.toolUsage);
    });
});
...
//# sourceMappingURL=Task.throttle.test.js.map