...
import { RooCodeEventName, TelemetryEventName, TaskStatus, getApiProtocol, getModelId, isIdleAsk, isInteractiveAsk, isResumableAsk, DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, DEFAULT_CHECKPOINT_TIMEOUT_SECONDS, MAX_CHECKPOINT_TIMEOUT_SECONDS, MIN_CHECKPOINT_TIMEOUT_SECONDS, ConsecutiveMistakeError, MAX_MCP_TOOLS_THRESHOLD, countEnabledMcpTools, } from "@roo-code/types";
...
export class Task extends EventEmitter {
    ...
    pushToolResultToUserContent(toolResult) {
        const existingResult = this.userMessageContent.find((block) => block.type === "tool_result" && block.tool_use_id === toolResult.tool_use_id);
        if (existingResult) {
            console.warn(`[Task#pushToolResultToUserContent] Skipping duplicate tool_result for tool_use_id: ${toolResult.tool_use_id}`);
            return false;
        }
     ...
     */
    backgroundServices;
    constructor({ provider, apiConfiguration, enableDiff = false, enableCheckpoints = true, checkpointTimeout = DEFAULT_CHECKPOINT_TIMEOUT_SECONDS, enableBridge = false, fuzzyMatchThreshold = 1.0, consecutiveMistakeLimit = DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, task, images, historyItem, experiments: experimentsConfig, startTask = true, rootTask, parentTask, taskNumber = -1, onCreated, initialTodos, workspacePath, initialStatus, mode, }) {
        super();
        ...
        this.browserSession = new BrowserSession(provider.context, (isActive) => {
        ...
        this.debouncedEmitTokenUsage = debounce((tokenUsage, toolUsage) => {
        ...
        }, this.TOKEN_USAGE_EMIT_INTERVAL_MS, { leading: true, trailing: true, maxWait: this.TOKEN_USAGE_EMIT_INTERVAL_MS });
        onCreated?.(this);
        if (startTask) {
            if (task || images) {
    ...
    setupProviderProfileChangeListener(provider) {
        ...
        this.providerProfileChangeListener = async () => {
            try {
                ...
                console.error(`[Task#${this.taskId}.${this.instanceId}] Failed to update API configuration on profile change:`, error);
            }
        };
    ...
    async condenseContext() {
        ...
        const { messages, summary, cost, newContextTokens = 0, error, condenseId, } = await summarizeConversation(this.apiConversationHistory, this.api, // Main API handler (fallback)
    ...
    async startTask(task, images) {
        try {
            ...
            if (enabledToolCount > MAX_MCP_TOOLS_THRESHOLD) {
                await this.say("too_many_tools_warning", JSON.stringify({
                    toolCount: enabledToolCount,
                    serverCount: enabledServerCount,
                    threshold: MAX_MCP_TOOLS_THRESHOLD,
                }), undefined, undefined, undefined, undefined, { isNonInteractive: true });
            }
            ...
            if (Array.isArray(lastUserMsg.content)) {
                ...
                const contentWithoutEnvDetails = lastUserMsg.content.filter((block) => {
                    if (block.type === "text" && typeof block.text === "string") {
                        const isEnvironmentDetailsBlock = block.text.trim().startsWith("<environment_details>") &&
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        while (stack.length > 0) {
            ...
            if (this.consecutiveMistakeLimit > 0 && this.consecutiveMistakeCount >= this.consecutiveMistakeLimit) {
                ...
                TelemetryService.instance.captureException(new ConsecutiveMistakeError(`Task reached consecutive mistake limit (${this.consecutiveMistakeLimit})`, this.taskId, this.consecutiveMistakeCount, this.consecutiveMistakeLimit, "no_tools_used", this.apiConfiguration.apiProvider, getModelId(this.apiConfiguration)));
            ...
            const shouldAddUserMessage = ((currentItem.retryAttempt ?? 0) === 0 && !isEmptyUserContent) || currentItem.userMessageWasRemoved;
            ...
            try {
                ...
                const updateApiReqMsg = (cancelReason, streamingFailedMessage) => {
                    ...
                    const costResult = apiProtocol === "anthropic"
                        ? calculateApiCostAnthropic(streamModelInfo, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens)
                                ...
                                for (const event of events) {
                                    if (event.type === "tool_call_start") {
                                        ...
                                        if (this.streamingToolCallIndices.has(event.id)) {
                                            console.warn(`[Task#${this.taskId}] Ignoring duplicate tool_call_start for ID: ${event.id} (tool: ${event.name})`);
                                            continue;
                                        }
                    ...
                    const drainStreamInBackgroundToFindAllUsage = async (apiReqIndex) => {
                        ...
                        const captureUsageData = async (tokens, messageIndex = apiReqIndex) => {
                            if (tokens.input > 0 ||
                                ...
                                const costResult = apiProtocol === "anthropic"
                                    ? calculateApiCostAnthropic(streamModelInfo, tokens.input, tokens.output, tokens.cacheWrite, tokens.cacheRead)
                                    : calculateApiCostOpenAI(streamModelInfo, tokens.input, tokens.output, tokens.cacheWrite, tokens.cacheRead);
                    ...
                    if (!this.abandoned) {
                        ...
                        if (this.abort) {
                            ...
                            if (currentRetryAttempt >= MAX_STREAMING_FAILURE_RETRIES) {
                                console.error(`[Task#${this.taskId}.${this.instanceId}] Stream failed after ${currentRetryAttempt} retries, giving up: ${streamingFailedMessage}`);
                                // Show error to user and stop retrying
                                await this.say("error", `Streaming failed after ${currentRetryAttempt} retries. Please check your connection or try a different model.\n\nError: ${streamingFailedMessage}`);
                                break;
                            }
                            // Stream failed - log the error and retry with the same content
                            // The existing rate limiting will prevent rapid retries
                            console.error(`[Task#${this.taskId}.${this.instanceId}] Stream failed (attempt ${currentRetryAttempt + 1}/${MAX_STREAMING_FAILURE_RETRIES}), will retry: ${streamingFailedMessage}`);
                ...
                const hasToolUses = this.assistantMessageContent.some((block) => block.type === "tool_use" || block.type === "mcp_tool_use");
                if (hasTextContent || hasToolUses) {
                    ...
                    const toolUseBlocks = this.assistantMessageContent.filter((block) => block.type === "tool_use" || block.type === "mcp_tool_use");
                        ...
                        if (block.type === "mcp_tool_use") {
                            ...
                            if (mcpBlock.id) {
                                ...
                                if (seenToolUseIds.has(sanitizedId)) {
                                    console.warn(`[Task#${this.taskId}] Pre-flight deduplication: Skipping duplicate MCP tool_use ID: ${sanitizedId} (tool: ${mcpBlock.name})`);
                                    continue;
                                }
                            ...
                            if (toolCallId) {
                                ...
                                // Pre-flight deduplication: Skip if we've already added this ID
                                if (seenToolUseIds.has(sanitizedId)) {
                                    console.warn(`[Task#${this.taskId}] Pre-flight deduplication: Skipping duplicate tool_use ID: ${sanitizedId} (tool: ${toolUse.name})`);
                                    continue;
                                }
                                seenToolUseIds.add(sanitizedId);
                    ...
                    const didToolUse = this.assistantMessageContent.some((block) => block.type === "tool_use" || block.type === "mcp_tool_use");
                    if (!didToolUse) {
                    ...
                    if (state?.autoApprovalEnabled) {
                        ...
                        await this.backoffAndAnnounce(currentItem.retryAttempt ?? 0, new Error("Unexpected API Response: The language model did not provide any assistant messages. This may indicate an issue with the API or the model's output."));
                        // Check if task was aborted during the backoff
                        if (this.abort) {
    ...
    async getSystemPrompt() {
        ...
        const state = await this.providerRef.deref()?.getState();
        const { browserViewportSize, mode, customModes, customModePrompts, customInstructions, experiments, enableMcpServerCreation, browserToolEnabled, language, maxConcurrentFileReads, maxReadFileLine, apiConfiguration, enableSubfolderRules, } = state ?? {};
        return await (async () => {
            const provider = this.providerRef.deref();
            if (!provider) {
                throw new Error("Provider not available");
            ...
            return SYSTEM_PROMPT(provider.context, this.cwd, canUseBrowserTool, mcpHub, this.diffStrategy, browserViewportSize ?? "900x600", mode ?? defaultModeSlug, customModePrompts, customModes, customInstructions, this.diffEnabled, experiments, enableMcpServerCreation, language, rooIgnoreInstructions, maxReadFileLine !== -1, {
                maxConcurrentFileReads: maxConcurrentFileReads ?? 5,
                todoListEnabled: apiConfiguration?.todoListEnabled ?? true,
                browserToolEnabled: browserToolEnabled ?? true,
                useAgentRules: vscode.workspace.getConfiguration(Package.name).get("useAgentRules") ?? true,
            ...
            this.api.getModel().id, provider.getSkillsManager());
        })();
    }
    ...
    async handleContextWindowExceededError() {
        ...
        if (truncateResult.summary) {
            ...
            await this.say("sliding_window_truncation", undefined /* text */, undefined /* images */, false /* partial */, undefined /* checkpoint */, undefined /* progressStatus */, { isNonInteractive: true } /* options */, undefined /* contextCondense */, contextTruncation);
        }
    ...
    async maybeWaitForProviderRateLimit(retryAttempt) {
        const state = await this.providerRef.deref()?.getState();
        const rateLimitSeconds = state?.apiConfiguration?.rateLimitSeconds ?? this.apiConfiguration?.rateLimitSeconds ?? 0;
        if (rateLimitSeconds <= 0 || !Task.lastGlobalApiRequestTime) {
            return;
        }
        const now = performance.now();
        const timeSinceLastRequest = now - Task.lastGlobalApiRequestTime;
        const rateLimitDelay = Math.ceil(Math.min(rateLimitSeconds, Math.max(0, rateLimitSeconds * 1000 - timeSinceLastRequest) / 1000));
    ...
    async *attemptApiRequest(retryAttempt = 0, options = {}) {
        const state = await this.providerRef.deref()?.getState();
        const { apiConfiguration, autoApprovalEnabled, requestDelaySeconds, mode, autoCondenseContext = true, autoCondenseContextPercent = 100, profileThresholds = {}, } = state ?? {};
    ...
    buildCleanConversationHistory(messages) {
        ...
        for (const msg of messages) {
            ...
            if (msg.role === "assistant") {
                ...
                const hasEncryptedReasoning = first && first.type === "reasoning" && typeof first.encrypted_content === "string";
    ...
    broadcastBrowserSessionUpdate() {
        ...
        try {
            ...
            const browserSessionStartIndex = this.clineMessages.findIndex((m) => m.ask === "browser_action_launch" ||
                (m.say === "browser_session_status" && m.text?.includes("opened")));
            const browserSessionMessages = browserSessionStartIndex !== -1 ? this.clineMessages.slice(browserSessionStartIndex) : [];
            const isBrowserSessionActive = this.browserSession?.isSessionActive() ?? false;
            // Update the panel asynchronously
            panelManager.updateBrowserSession(browserSessionMessages, isBrowserSessionActive).catch((error) => {
