 ...
 */
function findToolUseBlockById(message, toolUseId) {
    if (message.role !== "assistant" || typeof message.content === "string") {
        return undefined;
    }
    return message.content.find((block) => block.type === "tool_use" && block.id === toolUseId);
}
/**
 ...
 */
function getReasoningBlocks(message) {
    if (message.role !== "assistant" || typeof message.content === "string") {
        return [];
    }
    // Filter for reasoning blocks and cast to ContentBlockParam (the type field is compatible)
    return message.content.filter((block) => block.type === "reasoning");
}
/**
 ...
 */
export function getKeepMessagesWithToolBlocks(messages, keepCount) {
    ...
    const toolUseBlocksToPreserve = [];
    const reasoningBlocksToPreserve = [];
 ...
 */
export async function summarizeConversation(messages, apiHandler, systemPrompt, taskId, prevContextTokens, isAutomaticTrigger, customCondensingPrompt) {
    TelemetryService.instance.captureContextCondensed(taskId, isAutomaticTrigger ?? false, !!customCondensingPrompt?.trim());
    ...
    const { keepMessages, toolUseBlocksToPreserve, reasoningBlocksToPreserve } = getKeepMessagesWithToolBlocks(messages, N_MESSAGES_TO_KEEP);
    ...
    const textBlock = { type: "text", text: summary };
    let summaryContent;
    if (toolUseBlocksToPreserve.length > 0) {
        ...
        summaryContent = [syntheticReasoningBlock, textBlock];
    }
 ...
 */
export function getEffectiveApiHistory(messages) {
 ...
 */
export function cleanupAfterTruncation(messages) {
