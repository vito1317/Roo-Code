 ...
 */
export function injectSyntheticToolResults(messages) {
 ...
 */
export function extractCommandBlocks(message) {
 ...
 */
export async function summarizeConversation(messages, apiHandler, systemPrompt, taskId, isAutomaticTrigger, customCondensingPrompt, metadata, environmentDetails) {
    ...
    const requestMessages = maybeRemoveImageBlocks([...messagesWithToolResults, finalRequestMessage], apiHandler).map(({ role, content }) => ({ role, content }));
...
${commandBlocks}
    ...
    const contextBlocks = [systemPromptMessage, summaryMessage].flatMap((message) => typeof message.content === "string" ? [{ text: message.content, type: "text" }] : message.content);
