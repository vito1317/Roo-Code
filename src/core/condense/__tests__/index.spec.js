...
import { summarizeConversation, getMessagesSinceLastSummary, getEffectiveApiHistory, cleanupAfterTruncation, extractCommandBlocks, injectSyntheticToolResults, } from "../index";
...
describe("summarizeConversation", () => {
    ...
    it("should correctly format the request to the API", async () => {
        ...
        expect(mockApiHandler.createMessage).toHaveBeenCalledWith(expect.stringContaining("You are a helpful AI assistant tasked with summarizing conversations."), expect.any(Array), undefined);
...
describe("summarizeConversation with custom settings", () => {
    ...
    it("should use custom prompt when provided", async () => {
        ...
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, localTaskId, false, customPrompt);
    ...
    it("should use default systemPrompt when custom prompt is empty or not provided", async () => {
        ...
        expect(createMessageCalls.length).toBe(1);
        expect(createMessageCalls[0][0]).toContain("You are a helpful AI assistant tasked with summarizing conversations.");
        expect(createMessageCalls[0][0]).toContain("CRITICAL: This is a summarization-only request");
        // Reset mock and test with undefined
        vi.clearAllMocks();
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, localTaskId, false, undefined);
    ...
    it("should capture telemetry when using custom prompt", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, localTaskId, false, "Custom prompt");
        // Verify telemetry was called with custom prompt flag
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(localTaskId, false, true);
    });
    /**
     * Test that telemetry is called with isAutomaticTrigger flag
     */
    it("should capture telemetry with isAutomaticTrigger flag", async () => {
        await summarizeConversation(sampleMessages, mockMainApiHandler, defaultSystemPrompt, localTaskId, true, // isAutomaticTrigger
        "Custom prompt");
        // Verify telemetry was called with isAutomaticTrigger flag
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(localTaskId, true, // isAutomaticTrigger
        true);
    });
});
