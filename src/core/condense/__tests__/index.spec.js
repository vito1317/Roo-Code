...
import { summarizeConversation, getMessagesSinceLastSummary, getKeepMessagesWithToolBlocks, getEffectiveApiHistory, N_MESSAGES_TO_KEEP, } from "../index";
...
describe("getKeepMessagesWithToolBlocks", () => {
    ...
    it("should preserve tool_use blocks when first kept message has tool_result blocks", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Reading file..." }, toolUseBlock],
                ts: 4,
            },
            {
                role: "user",
                content: [toolResultBlock, { type: "text", text: "Continue" }],
                ts: 5,
            },
    ...
    it("should not preserve tool_use blocks when first kept message is assistant role", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Reading..." }, toolUseBlock],
                ts: 4,
            },
    ...
    it("should handle multiple tool_use blocks that need to be preserved", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Reading files..." }, toolUseBlock1, toolUseBlock2],
                ts: 2,
            },
            {
                role: "user",
    ...
    it("should preserve reasoning blocks alongside tool_use blocks for DeepSeek/Z.ai interleaved thinking", () => {
        ...
        const messages = [
            ...
            { role: "user", content: "Please read the file", ts: 3 },
            {
                ...
                content: [reasoningBlock, { type: "text", text: "Reading file..." }, toolUseBlock],
                ts: 4,
            },
            {
                role: "user",
        ...
        expect(result.reasoningBlocksToPreserve[0].type).toBe("reasoning");
        expect(result.reasoningBlocksToPreserve[0].text).toBe("Let me think about this step by step...");
    });
    ...
    it("should preserve tool_use when tool_result is in 3rd kept message and tool_use is at boundary edge", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Searching..." }, toolUseBlock],
                ts: 2,
            },
            ...
            {
                role: "user",
                content: [toolResultBlock, { type: "text", text: "Done" }],
                ts: 5,
            },
        ];
        const result = getKeepMessagesWithToolBlocks(messages, 3);
    ...
    it("should preserve multiple tool_uses when tool_results are in different kept messages", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Reading file 1..." }, toolUseBlock1],
                ts: 2,
            },
            { role: "user", content: "Some message", ts: 3 },
            {
                role: "assistant",
                content: [{ type: "text", text: "Reading file 2..." }, toolUseBlock2],
                ts: 4,
            },
            {
                role: "user",
                content: [toolResultBlock1, { type: "text", text: "First result" }],
                ts: 5,
            },
            {
                role: "user",
                content: [toolResultBlock2, { type: "text", text: "Second result" }],
                ts: 6,
            },
    ...
    it("should not duplicate tool_use blocks when same tool_result ID appears multiple times", () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Using tool..." }, toolUseBlock],
                ts: 2,
            },
            {
                role: "user",
...
describe("summarizeConversation", () => {
    ...
    it("should include the original first user message in summarization input", async () => {
        ...
        const hasInitialAsk = mockCallArgs.some((m) => m.role === "user" &&
            (typeof m.content === "string"
                ? m.content === "Initial ask"
                : Array.isArray(m.content) &&
                    m.content.some((b) => b.type === "text" && b.text === "Initial ask")));
    ...
    it("should return error when API handler is invalid", async () => {
        ...
        const result = await summarizeConversation(messages, invalidHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS);
    ...
    it("should append tool_use blocks to summary message when first kept message has tool_result blocks", async () => {
        ...
        // Create a stream with usage information
        const streamWithUsage = (async function* () {
            yield { type: "text", text: "Summary of conversation" };
            yield { type: "usage", totalCost: 0.05, outputTokens: 100 };
        })();
        mockApiHandler.createMessage = vi.fn().mockReturnValue(streamWithUsage);
        mockApiHandler.countTokens = vi.fn().mockImplementation(() => Promise.resolve(50));
        const result = await summarizeConversation(messages, mockApiHandler, defaultSystemPrompt, taskId, DEFAULT_PREV_CONTEXT_TOKENS);
        ...
        expect(content[0].type).toBe("reasoning"); // Synthetic reasoning for DeepSeek
        expect(content[1].type).toBe("text");
        expect(content[1].text).toBe("Summary of conversation");
        expect(content[2].type).toBe("tool_use");
        expect(content[2].id).toBe("toolu_123");
        expect(content[2].name).toBe("read_file");
    ...
    it("should include user tool_result message in summarize request when preserving tool_use blocks", async () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Running tool..." }, toolUseBlock],
                ts: 3,
            },
            {
                role: "user",
                content: [toolResultBlock, { type: "text", text: "Thanks" }],
                ts: 4,
            },
            { role: "assistant", content: "Anything else?", ts: 5 },
            { role: "user", content: "Nope", ts: 6 },
        ];
        let capturedRequestMessages;
        ...
        expect(assistantMessage.content.some((block) => block.type === "tool_use" && block.id === toolUseBlock.id)).toBe(true);
        expect(userMessage.role).toBe("user");
        expect(Array.isArray(userMessage.content)).toBe(true);
        expect(userMessage.content.some((block) => block.type === "tool_result" && block.tool_use_id === toolUseBlock.id)).toBe(true);
    });
    it("should append multiple tool_use blocks for parallel tool calls", async () => {
        ...
        const messages = [
            ...
            {
                role: "assistant",
                content: [{ type: "text", text: "Launching parallel tools" }, toolUseBlockA, toolUseBlockB],
                ts: 3,
            },
            {
                role: "user",
                content: [
                    { type: "tool_result", tool_use_id: "toolu_parallel_1", content: "result A" },
                    { type: "tool_result", tool_use_id: "toolu_parallel_2", content: "result B" },
        ...
        expect(summaryContent[0].type).toBe("reasoning");
    ...
    it("should preserve reasoning blocks in summary message for DeepSeek/Z.ai interleaved thinking", async () => {
        ...
        expect(content[1].text).toBe("Let me think about this step by step...");
        // Third block should be text (the summary)
        expect(content[2].type).toBe("text");
        expect(content[2].text).toBe("Summary of conversation");
        // Fourth block should be tool_use
        expect(content[3].type).toBe("tool_use");
        expect(content[3].id).toBe("toolu_deepseek_reason");
        expect(result.error).toBeUndefined();
    });
    it("should include synthetic reasoning block in summary for DeepSeek-reasoner compatibility even without tool_use blocks", async () => {
        ...
        const streamWithUsage = (async function* () {
            yield { type: "text", text: "Summary: User requested jokes." };
            yield { type: "usage", totalCost: 0.05, outputTokens: 100 };
        })();
        mockApiHandler.createMessage = vi.fn().mockReturnValue(streamWithUsage);
        mockApiHandler.countTokens = vi.fn().mockImplementation(() => Promise.resolve(50));
...
describe("summarizeConversation with custom settings", () => {
    ...
    it("should capture telemetry when using custom prompt", async () => {
        ...
        expect(TelemetryService.instance.captureContextCondensed).toHaveBeenCalledWith(taskId, false, true);
    });
    /**
