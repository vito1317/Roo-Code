 ...
 */
import { createHandoffContext, validateHandoffContext, getHandoffSummary, } from "./HandoffContext";
import { isSentinelAgent } from "./personas";
...
export class SentinelStateMachine {
    ...
    transitions = [
        ...
        // Architect completes planning → Designer (if UI design needed) or Builder
        {
            ...
            condition: (ctx) => {
                ...
                const plan = ctx.architectPlan;
                const isTruthy = (val) => val === true || val === "true" || val === 1 || val === "1";
                if (isTruthy(plan?.needsDesign) || isTruthy(plan?.needs_design))
                    return true;
                if (isTruthy(plan?.hasUI) || isTruthy(plan?.has_ui) || isTruthy(plan?.hasUi))
                    return true;
                if (isTruthy(plan?.useFigma) || isTruthy(plan?.use_figma))
                    return true;
                // Check notes for keywords
                const notes = ctx.previousAgentNotes?.toLowerCase() || "";
                if (notes.includes("figma") || notes.includes("designer") || notes.includes("ui design"))
                    return true;
                return false;
            },
    ...
    constructor(task, provider, config = {}) {
     ...
     */
    async handleAgentCompletion(handoffData) {
        ...
        if (nextState === AgentState.BUILDER) {
            ...
            if (fromState === AgentState.QA_ENGINEER || fromState === AgentState.ARCHITECT_REVIEW_TESTS) {
                ...
                if (this.qaRejectionCount >= this.config.maxQARetries) {
                    return this.triggerHumanIntervention(`QA tests failed ${this.qaRejectionCount} times. Human intervention required.`);
                }
            }
                ...
                if (this.securityRejectionCount >= this.config.maxSecurityRetries) {
                    return this.triggerHumanIntervention(`Security audit failed ${this.securityRejectionCount} times. Human intervention required.`);
                }
            }
        }
     ...
     * Determine next state based on current state and handoff data
     */
    determineNextState(handoffData) {
        switch (this.currentState) {
            ...
            case AgentState.ARCHITECT: {
                ...
                const plan = handoffData.architectPlan;
     ...
     */
    async switchToAgentMode(agentState) {
     ...
     */
    async openFigmaPreviewForDesigner(provider) {
        try {
     ...
     */
    async triggerHumanIntervention(reason) {
        const fromState = this.currentState;
        // Record failure
        if (this.currentContext) {
            const failure = {
    ...
    async generateWalkthrough() {
        ...
        try {
            ...
            try {
                ...
                const screenshots = files.filter(f => f.endsWith('.png') || f.endsWith('.jpg') || f.endsWith('.jpeg') || f.endsWith('.webp')).filter(f => f.includes('screenshot') || f.includes('test') || f.includes('browser'));
    ...
    notifyWebview(state) {
        ...
        try {
            ...
            if (this.currentContext) {
                ...
                const fromDisplayName = this.getAgentDisplayName(ctx.fromAgent) || ctx.fromAgent;
                const toDisplayName = this.getAgentDisplayName(ctx.toAgent) || ctx.toAgent;
                lastHandoff = {
 ...
 */
export function createSentinelFSM(task, provider, config) {
