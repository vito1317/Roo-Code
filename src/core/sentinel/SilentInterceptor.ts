...
/**
 * Pattern to match and resolver to answer
 */
interface InterceptPattern {
	regex: RegExp
	resolver: (cwd: string) => Promise<string>
	description: string
}
...
export class SilentInterceptor {
	private patterns: InterceptPattern[]
	 ...
	 */
	async interceptAsk(question: string, cwd: string): Promise<string | null> {
	 ...
	 */
	private async readPackageJson(cwd: string): Promise<PackageJson | null> {
	...
	private async detectFramework(cwd: string): Promise<string> {
		...
		else if (deps["nuxt"]) frameworks.push(`Nuxt ${deps["nuxt"]}`)
		else if (deps["@angular/core"]) frameworks.push(`Angular ${deps["@angular/core"]}`)
		else if (deps["vue"]) frameworks.push(`Vue.js ${deps["vue"]}`)
		else if (deps["react"]) frameworks.push(`React ${deps["react"]}`)
		else if (deps["svelte"]) frameworks.push(`Svelte ${deps["svelte"]}`)
		...
		if (deps["hono"]) frameworks.push(`Hono ${deps["hono"]}`)
		...
		// Mobile
		if (deps["react-native"]) frameworks.push(`React Native ${deps["react-native"]}`)
		if (deps["expo"]) frameworks.push(`Expo ${deps["expo"]}`)
	 ...
	 */
	private async generateProjectTree(cwd: string, depth: number = 2): Promise<string> {
		...
		const walk = async (dir: string, prefix: string, currentDepth: number): Promise<void> => {
			...
			try {
				...
				const filtered = entries.filter(
					(e) => !e.name.startsWith(".") && !["node_modules", "dist", "build", ".git"].includes(e.name),
				)
	 ...
	 */
	private async getAvailableScripts(cwd: string): Promise<string> {
		const pkg = await this.readPackageJson(cwd)
		if (!pkg?.scripts) {
	 ...
	 */
	private async detectTestFramework(cwd: string): Promise<string> {
		const pkg = await this.readPackageJson(cwd)
		if (!pkg) {
		...
		if (deps["playwright"]) frameworks.push(`Playwright ${deps["playwright"]}`)
	 ...
	 */
	private async getTsConfig(cwd: string): Promise<string> {
		try {
	 ...
	 */
	private async detectDevPort(cwd: string): Promise<string> {
		const pkg = await this.readPackageJson(cwd)
		if (!pkg?.scripts) {
		...
		const deps = { ...pkg.dependencies, ...pkg.devDependencies }
		if (deps["next"]) return "Development port: 3000 (Next.js default)"
		if (deps["vite"]) return "Development port: 5173 (Vite default)"
		if (deps["@angular/core"]) return "Development port: 4200 (Angular default)"
		if (deps["vue"]) return "Development port: 5173 (Vue/Vite default) or 8080 (Vue CLI)"
		if (deps["react-scripts"]) return "Development port: 3000 (Create React App default)"
	 ...
	 */
	private async getEntryPoint(cwd: string): Promise<string> {
		const pkg = await this.readPackageJson(cwd)
		if (pkg?.main) {
 ...
 */
let interceptorInstance: SilentInterceptor | null = null

/**
 * Get or create the singleton silent interceptor
 */
export function getSilentInterceptor(): SilentInterceptor {
