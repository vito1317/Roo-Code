...
- 你的計畫必須足夠詳細，讓 Builder Agent 可以直接執行`,
    ...
    groups: ["read", "edit"], // Architect reads and creates plan files
    handoffOutputSchema: {
        type: "json",
 ...
 */
export const QA_ENGINEER_AGENT = {
    slug: "sentinel-qa",
    name: "🟨 QA Engineer",
...
⚠️ **注意**：你的問題會自動路由給 Architect Agent 回答，不會打擾用戶！`,
  ...
  "recommendation": "approve|fix_required|reject"
    ...
    customInstructions: "## Security Checklist\n\n" +
        "### SQL Injection\n" +
...
`;
            ...
            // Form-like UIs
            else if (lowerRequest.includes("表單") ||
...
`;
            ...
            // Dashboard/Stats UIs
            else if (lowerRequest.includes("儀表板") ||
...
`;
            ...
            // Navigation/Menu UIs
            else if (lowerRequest.includes("導航") ||
 ...
 */
export const DESIGN_REVIEW_AGENT = {
    slug: "sentinel-design-review",
    name: "🔎 Design Review",
    roleDefinition: "You are Roo, the Design Review Agent in Sentinel Edition. " +
  ...
  "missingComponents": []
    ...
    customInstructions: "**⛔ 重要限制 - 你不能創建 UI 元素！**\n\n" +
        "你的職責只是**驗證**設計，不是創建設計。\n" +
 ...
 */
export function getAgentPersona(slug) {
    return SENTINEL_AGENTS[slug];
}
/**
 * Get the next agent in the workflow
 */
export function getNextAgent(currentSlug, success) {
 ...
 */
export function isSentinelAgent(slug) {
 ...
 */
export function resolveCustomInstructions(agent, context = {}) {
 ...
 */
export function getSentinelModesConfig() {
 ...
 */
export function getSentinelModeConfigWithContext(slug, context) {
