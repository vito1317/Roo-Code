...
describe("ClineProvider flicker-free cancel", () => {
    ...
    beforeEach(() => {
        ...
        provider.getTaskWithId = vi.fn().mockImplementation((id) => Promise.resolve({
            historyItem: {
                id,
                number: 1,
                ts: Date.now(),
        ...
        vi.mocked(Task).mockImplementation(() => mockTask2);
    });
    it("should not remove current task from stack when rehydrating same taskId", async () => {
        ...
        expect(provider.clineStack[0]).toBe(mockTask2);
    ...
    it("should maintain task stack integrity during flicker-free replacement", async () => {
        ...
        expect(provider.clineStack).toHaveLength(2);
        expect(provider.clineStack[0]).toBe(mockParentTask);
        expect(provider.clineStack[1]).toBe(mockTask2);
    });
});
//# sourceMappingURL=ClineProvider.flicker-free-cancel.spec.js.map