...
describe("ClineProvider Task History Synchronization", () => {
    ...
    describe("updateTaskHistory", () => {
        ...
        it("updates existing task in history", async () => {
            ...
            expect(mockContext.globalState.update).toHaveBeenCalledWith("taskHistory", expect.arrayContaining([expect.objectContaining({ id: "task-update", task: "Updated task" })]));
            // Should not have duplicates
            const allCalls = mockContext.globalState.update.mock.calls;
            const lastUpdateCall = allCalls.find((call) => call[0] === "taskHistory");
            const historyArray = lastUpdateCall?.[1];
            const matchingItems = historyArray?.filter((item) => item.id === "task-update");
    ...
    describe("broadcastTaskHistoryUpdate", () => {
        ...
        it("reads from global state when no history is provided", async () => {
            ...
            const now = Date.now();
            const stateHistory = [createHistoryItem({ id: "from-state", ts: now, task: "State task" })];
...
//# sourceMappingURL=ClineProvider.taskHistory.spec.js.map