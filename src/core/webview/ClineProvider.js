...
import { RooCodeEventName, requestyDefaultModelId, openRouterDefaultModelId, DEFAULT_TERMINAL_OUTPUT_CHARACTER_LIMIT, DEFAULT_WRITE_DELAY_MS, ORGANIZATION_ALLOW_ALL, DEFAULT_MODES, DEFAULT_CHECKPOINT_TIMEOUT_SECONDS, getModelId, } from "@roo-code/types";
import { aggregateTaskCostsRecursive } from "./aggregateTaskCosts";
...
export class ClineProvider extends EventEmitter {
    ...
    constructor(context, outputChannel, renderContext = "sidebar", contextProxy, mdmService) {
        ...
        this.taskCreationCallback = (instance) => {
            ...
            const onTaskTokenUsageUpdated = (taskId, tokenUsage, toolUsage) => this.emit(RooCodeEventName.TaskTokenUsageUpdated, taskId, tokenUsage, toolUsage);
    ...
    */
    ...
    async resolveWebviewView(webviewView) {
        ...
        // global state values.
        this.getState().then(({ terminalShellIntegrationTimeout = Terminal.defaultShellIntegrationTimeout, terminalShellIntegrationDisabled = false, terminalCommandDelay = 0, terminalZshClearEolMark = true, terminalZshOhMy = false, terminalZshP10k = false, terminalPowershellCounter = false, terminalZdotdir = false, }) => {
        ...
        if (historyItem.apiConfigName) {
            ...
            if (profile?.name) {
                try {
                    ...
                    // Log the error but continue with task restoration.
                    this.log(`Failed to restore API configuration '${historyItem.apiConfigName}' for task: ${error instanceof Error ? error.message : String(error)}. Continuing with current configuration.`);
                }
            }
            else {
    ...
    async handleModeSwitch(newMode) {
        ...
        if (task && newMode.startsWith("sentinel-")) {
            ...
            const currentAgent = (modeToAgentState[newMode] || "IDLE");
    ...
    async persistStickyProviderProfileToCurrentTask(apiConfigName) {
        ...
        try {
            ...
            this.log(`Failed to persist provider profile switch for task ${task.taskId}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async getTaskWithAggregatedCosts(taskId) {
        ...
        const aggregatedCosts = await aggregateTaskCostsRecursive(taskId, async (id) => {
    ...
    async getState() {
        ...
        try {
            ...
            console.error(`[getState] failed to get public sharing enabled state: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        return {
            ...
            codebaseIndexConfig: {
                ...
                codebaseIndexOpenRouterSpecificProvider: stateValues.codebaseIndexConfig?.codebaseIndexOpenRouterSpecificProvider,
            },
    ...
    async broadcastTaskHistoryUpdate(history) {
        ...
        const taskHistory = history ?? this.getGlobalState("taskHistory") ?? [];
    ...
    async delegateParentAndOpenChild(params) {
        ...
        if (parent.taskId !== parentTaskId) {
            throw new Error(`[delegateParentAndOpenChild] Parent mismatch: expected ${parentTaskId}, current ${parent.taskId}`);
        }
        ...
        try {
            ...
            this.log(`[delegateParentAndOpenChild] Error flushing pending tool results (non-fatal): ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        try {
            ...
            this.log(`[delegateParentAndOpenChild] Error during parent disposal (non-fatal): ${error instanceof Error ? error.message : String(error)}`);
        ...
        try {
            ...
            this.log(`[delegateParentAndOpenChild] handleModeSwitch failed for mode '${mode}': ${e?.message ?? String(e)}`);
        }
        ...
        try {
            ...
            this.log(`[delegateParentAndOpenChild] Failed to persist parent metadata for ${parentTaskId} -> ${child.taskId}: ${err?.message ?? String(err)}`);
        }
    ...
    async reopenParentFromDelegation(params) {
        ...
        if (!Array.isArray(parentClineMessages))
            parentClineMessages = [];
        ...
        try {
            ...
            this.log(`[reopenParentFromDelegation] Failed to persist child completed status for ${childTaskId}: ${err?.message ?? String(err)}`);
        }
