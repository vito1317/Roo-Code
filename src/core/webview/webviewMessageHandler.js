...
import { TelemetryEventName, checkoutDiffPayloadSchema, checkoutRestorePayloadSchema, } from "@roo-code/types";
...
export const webviewMessageHandler = async (provider, message, marketplaceManager) => {
    ...
    const handleDeleteMessageConfirm = async (messageTs, restoreCheckpoint) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(t("common:errors.message.error_deleting_message", {
    ...
    const handleEditMessageConfirm = async (messageTs, editedContent, restoreCheckpoint, images) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(t("common:errors.message.error_editing_message", {
                error: error instanceof Error ? error.message : String(error),
            }));
        }
    };
    ...
    switch (message.type) {
            ...
            if (message.updatedSettings) {
                for (const [key, value] of Object.entries(message.updatedSettings)) {
                    ...
                    if (key === "language") {
                        ...
                        if (value !== undefined) {
                            Terminal.setShellIntegrationTimeout(value);
                        }
                    }
                    else if (key === "terminalShellIntegrationDisabled") {
                        if (value !== undefined) {
                            Terminal.setShellIntegrationDisabled(value);
                        }
                    }
                    else if (key === "terminalCommandDelay") {
                        if (value !== undefined) {
                            Terminal.setCommandDelay(value);
                        }
                    }
                    else if (key === "terminalPowershellCounter") {
                        if (value !== undefined) {
                            Terminal.setPowershellCounter(value);
                        }
                    }
                    else if (key === "terminalZshClearEolMark") {
                        if (value !== undefined) {
                            Terminal.setTerminalZshClearEolMark(value);
                        }
                    }
                    else if (key === "terminalZshOhMy") {
                        if (value !== undefined) {
                            Terminal.setTerminalZshOhMy(value);
                        }
                    }
                    else if (key === "terminalZshP10k") {
                        if (value !== undefined) {
                            Terminal.setTerminalZshP10k(value);
                        }
                    }
                    else if (key === "terminalZdotdir") {
                        if (value !== undefined) {
                            Terminal.setTerminalZdotdir(value);
                        }
                    }
                    else if (key === "terminalCompressProgressBar") {
                        if (value !== undefined) {
                            Terminal.setCompressProgressBar(value);
                        }
                    }
        ...
        case "flushRouterModels":
            ...
            await flushModels({ provider: routerNameFlush }, true);
            break;
        ...
        case "refreshBrowserSessionPanel":
            {
                ...
                if (task) {
                    ...
                    const browserSessionStartIndex = messages.findIndex((m) => m.ask === "browser_action_launch" ||
                        (m.say === "browser_session_status" && m.text?.includes("opened")));
                    const browserSessionMessages = browserSessionStartIndex !== -1 ? messages.slice(browserSessionStartIndex) : [];
            ...
            if (message.slug) {
                try {
                    ...
                    if (result.success && result.yaml) {
                        const defaultUri = await resolveDefaultSaveUri(provider.contextProxy, "lastModeExportPath", `${message.slug}-export.yaml`, {
                            useWorkspace: true,
                            fallbackDir: path.join(os.homedir(), "Downloads"),
                        });
                        // Show save dialog
        ...
        case "rooCloudManualUrl": {
            try {
                ...
                await CloudService.instance.handleAuthCallback(code, state, organizationId === "null" ? null : organizationId);
                await provider.postStateToWebview();
            }
            catch (error) {
        ...
        case "listWorktrees": {
            try {
                const { worktrees, isGitRepo, isMultiRoot, isSubfolder, gitRootPath, error } = await handleListWorktrees(provider);
                await provider.postMessageToWebview({
                    type: "worktreeList",
                    worktrees,
                    isGitRepo,
        ...
        case "deleteWorktree": {
            try {
                const { success, message: text } = await handleDeleteWorktree(provider, message.worktreePath, message.worktreeForce ?? false);
                await provider.postMessageToWebview({ type: "worktreeResult", success, text });
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
        ...
        case "switchWorktree": {
            try {
                const { success, message: text } = await handleSwitchWorktree(provider, message.worktreePath, message.worktreeNewWindow ?? true);
                await provider.postMessageToWebview({ type: "worktreeResult", success, text });
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
        ...
        case "createWorktreeInclude": {
            try {
                const { success, message: text } = await handleCreateWorktreeInclude(provider, message.worktreeIncludeContent ?? "");
                await provider.postMessageToWebview({ type: "worktreeResult", success, text });
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
