...
export async function handleListWorktrees(provider) {
...
export async function handleCreateWorktree(provider, options, onCopyProgress) {
    const cwd = provider.cwd;
    const isGitRepo = await worktreeService.checkGitRepo(cwd);
    if (!isGitRepo) {
        return {
    ...
    if (result.success && result.worktree) {
        try {
            const copiedItems = await worktreeIncludeService.copyWorktreeIncludeFiles(cwd, result.worktree.path, onCopyProgress);
...
export async function handleDeleteWorktree(provider, worktreePath, force = false) {
    const cwd = provider.cwd;
    return worktreeService.deleteWorktree(cwd, worktreePath, force);
}
export async function handleSwitchWorktree(provider, worktreePath, newWindow) {
    try {
...
export async function handleGetAvailableBranches(provider) {
    const cwd = provider.cwd;
...
export async function handleGetWorktreeDefaults(provider) {
...
export async function handleGetWorktreeIncludeStatus(provider) {
    const cwd = provider.cwd;
    return worktreeIncludeService.getStatus(cwd);
}
export async function handleCheckBranchWorktreeInclude(provider, branch) {
    const cwd = provider.cwd;
    return worktreeIncludeService.branchHasWorktreeInclude(cwd, branch);
}
export async function handleCreateWorktreeInclude(provider, content) {
    const cwd = provider.cwd;
    try {
...
export async function handleCheckoutBranch(provider, branch) {
    const cwd = provider.cwd;
