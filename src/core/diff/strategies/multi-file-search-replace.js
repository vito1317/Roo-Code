...
Each file requires its own path, start_line, and diff elements.
    ...
    validateMarkerSequencing(diffContent) {
        ...
        const reportMergeConflictError = (found, _expected) => ({
            success: false,
            error: `ERROR: Special marker '${found}' found in your diff content at line ${state.line}:\n` +
                "\n" +
        ...
        const reportInvalidDiffError = (found, expected) => ({
            success: false,
            error: `ERROR: Diff block is malformed: marker '${found}' found in your diff content at line ${state.line}. Expected: ${expected}\n` +
                "\n" +
                "CORRECT FORMAT:\n\n" +
                "<<<<<<< SEARCH\n" +
                ":start_line: (required) The line number of original content where the search block starts.\n" +
        ...
        const reportLineMarkerInReplaceError = (marker) => ({
            success: false,
            error: `ERROR: Invalid line marker '${marker}' found in REPLACE section at line ${state.line}\n` +
                "\n" +
    ...
    async applySingleDiff(originalContent, diffContent, _paramStartLine) {
        ...
        */
        ...
        for (const replacement of replacements) {
            ...
            const hasAllLineNumbers = (everyLineHasLineNumbers(searchContent) && everyLineHasLineNumbers(replaceContent)) ||
            ...
            if (matchIndex === -1) {
                const { bestScore, bestMatchIndex, bestMatchContent: midContent, } = fuzzySearch(resultLines, searchChunk, searchStartIndex, searchEndIndex);
            ...
            if (matchIndex === -1 || bestMatchScore < this.fuzzyThreshold) {
                ...
                const { bestScore, bestMatchIndex, bestMatchContent: aggContent, } = fuzzySearch(resultLines, aggressiveSearchChunk, searchStartIndex, searchEndIndex);
                if (bestMatchIndex !== -1 && bestScore >= this.fuzzyThreshold) {
                    ...
                    const originalContentSection = startLine !== undefined && endLine !== undefined
                        ? `\n\nOriginal Content:\n${addLineNumbers(resultLines
                            .slice(Math.max(0, startLine - 1 - this.bufferLines), Math.min(resultLines.length, endLine + this.bufferLines))
...
//# sourceMappingURL=multi-file-search-replace.js.map