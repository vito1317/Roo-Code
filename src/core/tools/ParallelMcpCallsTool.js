...
export class ParallelMcpCallsTool extends BaseTool {
    ...
    async execute(params, task, callbacks) {
        ...
        try {
            ...
            try {
                ...
                task.recordToolError("parallel_mcp_calls");
                task.didToolFailInCurrentTurn = true;
                pushToolResult(formatResponse.toolError("Invalid calls format. Expected a JSON array of MCP calls.\n\n" +
            ...
            if (parsedCalls.length > MAX_BATCH_SIZE) {
                await task.say("text", `⚠️ 收到 ${parsedCalls.length} 個調用，將自動分批處理（每批最多 ${MAX_BATCH_SIZE} 個）`);
            }
            ...
            for (const call of parsedCalls) {
                if (!call.tool) {
                    ...
                    task.recordToolError("parallel_mcp_calls");
                    task.didToolFailInCurrentTurn = true;
                    pushToolResult(formatResponse.toolError(`Each call must have 'tool'. Invalid call: ${JSON.stringify(call)}`));
                    return;
                }
            ...
            for (let batchStart = 0; batchStart < parsedCalls.length; batchStart += MAX_BATCH_SIZE) {
                ...
                const batchResults = await Promise.all(batch.map(async (call, localIndex) => {
            ...
            // Report results
            if (failedCount === 0) {
                await task.say("text", `✅ All ${parsedCalls.length} MCP calls completed successfully in ${duration}ms!`);
            }
            else {
                const failedResults = allResults.filter((r) => !r.success);
                await task.say("text", `⚠️ ${successCount}/${parsedCalls.length} calls succeeded, ${failedCount} failed.\n\n` +
                    `Failed calls:\n` +
                    failedResults.map((r) => `  • [${r.index}] ${r.tool}: ${r.error}`).join("\n"));
            }
            pushToolResult(formatResponse.toolResult(`Parallel MCP calls completed.\n` +
                `- Success: ${successCount}\n` +
                `- Failed: ${failedCount}\n` +
                `- Duration: ${duration}ms`));
        }
        catch (error) {
            await handleError("executing parallel MCP calls", error);
        }
    }
...
//# sourceMappingURL=ParallelMcpCallsTool.js.map