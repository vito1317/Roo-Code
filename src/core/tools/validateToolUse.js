...
import { FileRestrictionError, getModeBySlug, getGroupName } from "../../shared/modes";
 ...
 */
export function isValidToolName(toolName, experiments) {
...
export function validateToolUse(toolName, mode, customModes, toolRequirements, toolParams, experiments, includedTools) {
    ...
    if (!isValidToolName(toolName, experiments)) {
        throw new Error(`Unknown tool "${toolName}". This tool does not exist. Please use one of the available tools: ${validToolNames.join(", ")}.`);
    }
    // Then check if the tool is allowed for the current mode
    if (!isToolAllowedForMode(toolName, mode, customModes ?? [], toolRequirements, toolParams, experiments, includedTools)) {
