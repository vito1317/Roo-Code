...
export class AttemptCompletionTool extends BaseTool {
    ...
    async execute(params, task, callbacks) {
        ...
        if (preventCompletionWithOpenTodos && hasIncompleteTodos) {
            ...
            pushToolResult(formatResponse.toolError("Cannot complete task while there are incomplete todos. Please finish all todos before attempting completion."));
            return;
        }
        try {
            if (!result) {
            ...
            if (sentinelFSM && sentinelFSM.isActive()) {
                ...
                if (isFinalReview) {
                    ...
                    await task.say("text", `🎉 **Final Review Complete!** Transitioning to COMPLETED state and generating walkthrough...`);
                    ...
                    if (transitionResult.success) {
                        await task.say("text", `✅ **Sentinel Workflow Complete!**`);
                    }
                    else {
                        await task.say("text", `⚠️ **Workflow transition failed:** ${transitionResult.error}`);
                    }
                    ...
                    await task.say("text", `\ud83d\udd04 **Sentinel Mode:** Intercepting completion from ${currentState}. Initiating handoff...`);
                    ...
                    if (transitionResult.success) {
                        ...
                        const handoffMessage = `# 🔄 Sentinel Handoff\n\n` +
                        ...
                        await task.say("text", `✅ **Sentinel Handoff Complete**\n\n` +
                ...
                if (provider) {
                    try {
                        ...
                        if (status === "completed") {
                            ...
                            const delegated = await this.delegateToParent(task, result, provider, askFinishSubTaskApproval, pushToolResult);
                            if (delegated)
                                return;
                        }
                        else {
                            ...
                            console.error(`[AttemptCompletionTool] Unexpected child task status "${status}" for task ${task.taskId}. ` +
                        ...
                        console.error(`[AttemptCompletionTool] Failed to get history for task ${task.taskId}: ${err?.message ?? String(err)}. ` +
                            `Skipping delegation.`);
                        // Fall through to normal completion ask flow
                    }
                }
     ...
     */
    extractHandoffContextFromResult(result, currentState) {
     ...
     */
    buildHandoffDataFromParsed(parsed, currentState, originalResult) {
        const base = {
        ...
        switch (currentState) {
            case "sentinel-architect":
                return {
                    ...base,
                    architectPlan: parsed,
                };
            case "sentinel-builder":
                return {
                    ...base,
                    builderTestContext: parsed,
                };
            case "sentinel-qa":
                return {
                    ...base,
                    qaAuditContext: parsed,
                };
            case "sentinel-security":
                return {
                    ...base,
                    sentinelResult: parsed,
                };
            default:
                return base;
        }
