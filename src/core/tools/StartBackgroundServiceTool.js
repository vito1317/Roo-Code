 ...
 */
export class StartBackgroundServiceTool extends BaseTool {
    ...
    async execute(params, task, callbacks) {
        ...
        const { command, port, workingDirectory = task.cwd, healthCheckPath = "/", timeout = 30000, env = {}, } = params;
        try {
            ...
            if (task.backgroundServices?.has(effectivePort)) {
                ...
                pushToolResult(`Port ${effectivePort} is already in use by a background service (PID: ${existing.pid}). ` +
            ...
            // Show status
            await task.say("text", `🚀 Starting background service: \`${command}\` on port ${effectivePort}...`);
            // Spawn the process
            ...
            if (isReady) {
                pushToolResult(`✅ Background service started successfully!\n\n` +
                    `- **Command:** \`${command}\`\n` +
                    `- **PID:** ${child.pid}\n` +
                    `- **Port:** ${port}\n` +
                    `- **URL:** ${serviceInfo.url}\n` +
                ...
                pushToolResult(`⚠️ Background service started but health check timed out.\n\n` +
                    `- **Command:** \`${command}\`\n` +
                    `- **PID:** ${child.pid}\n` +
                    `- **Port:** ${port}\n` +
                    `- **Timeout:** ${timeout}ms\n\n` +
            ...
            await handleError("starting background service", error);
        }
    }
    /**
     * Spawn a detached process that won't block the conversation
     */
    spawnDetached(command, workingDirectory, env) {
     ...
     */
    async pollForReadiness(url, timeout) {
 ...
 */
export async function stopBackgroundService(task, port) {
 ...
 */
export async function stopAllBackgroundServices(task) {
    if (!task.backgroundServices)
        return;
...
//# sourceMappingURL=StartBackgroundServiceTool.js.map