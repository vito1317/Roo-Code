...
export class WriteToFileTool extends BaseTool {
    ...
    async execute(params, task, callbacks) {
        ...
        try {
                ...
                try {
                    ...
                    for (const group of vscode.window.tabGroups.all) {
                        for (const tab of group.tabs) {
                            if (tab.input && tab.input.uri?.fsPath === absolutePath) {
            ...
            if (isPreventFocusDisruptionEnabled) {
                ...
                await task.diffViewProvider.update(everyLineHasLineNumbers(newContent) ? stripLineNumbers(newContent) : newContent, true);
                await delay(300);
    ...
    async handlePartial(task, block) {
        ...
        if (newContent) {
            ...
            await task.diffViewProvider.update(everyLineHasLineNumbers(newContent) ? stripLineNumbers(newContent) : newContent, false);
        }
    }
}
