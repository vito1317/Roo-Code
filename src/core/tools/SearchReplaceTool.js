    ...
    async execute(params, task, callbacks) {
        ...
        try {
            ...
            if (matchCount === 0) {
                ...
                pushToolResult(formatResponse.toolError(`No match found for the specified 'old_string'. Please ensure it matches the file contents exactly, including whitespace and indentation.`));
                return;
            }
            if (matchCount > 1) {
                task.consecutiveMistakeCount++;
                task.recordToolError("search_replace", "multiple_matches");
                pushToolResult(formatResponse.toolError(`Found ${matchCount} matches for the specified 'old_string'. This tool can only replace ONE occurrence at a time. Please provide more context (3-5 lines before and after) to uniquely identify the specific instance you want to change.`));
                return;
            }
...
//# sourceMappingURL=SearchReplaceTool.js.map