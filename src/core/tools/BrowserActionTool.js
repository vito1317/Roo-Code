...
export async function browserActionTool(cline, block, askApproval, handleError, pushToolResult) {
    ...
    try {
        if (block.partial) {
            if (action === "launch") {
                ...
                await cline.say("browser_action", JSON.stringify({
                    action: action,
            ...
            if (action === "launch") {
                ...
                if (action === "click" || action === "hover") {
                    ...
                    try {
                        ...
                        cline.recordToolError("browser_action");
                        cline.didToolFailInCurrentTurn = true;
                        pushToolResult(await cline.sayAndCreateMissingParamError("browser_action", "coordinate", error instanceof Error ? error.message : String(error)));
                        return;
                    }
                }
            ...
            switch (action) {
                ...
                case "screenshot": {
                    ...
                    let messageText = action === "screenshot"
