...
describe("searchAndReplaceTool", () => {
     ...
     */
    async function executeSearchAndReplaceTool(params = {}, options = {}) {
        ...
        const nativeArgs = {
            ...
            operations: typeof fullParams.operations === "string" ? JSON.parse(fullParams.operations) : fullParams.operations,
        };
        const toolUse = {
            type: "tool_use",
        ...
        await searchAndReplaceTool.handle(mockTask, toolUse, {
    ...
    describe("search and replace logic", () => {
        it("returns error when no match is found", async () => {
            const result = await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: "NonExistent", replace: "New" }]) }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
        ...
        it("returns error when multiple matches are found", async () => {
            const result = await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: "Line", replace: "Row" }]) }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
            expect(result).toContain("3 matches");
            expect(mockTask.consecutiveMistakeCount).toBe(1);
        });
        it("successfully replaces single unique match", async () => {
            await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: "Line 2", replace: "Modified Line 2" }]) }, { fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("CRLF normalization", () => {
        it("normalizes CRLF to LF when reading file", async () => {
            ...
            await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: "Line 2", replace: "Modified Line 2" }]) }, { fileContent: contentWithCRLF });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
        it("normalizes CRLF in search string to match LF-normalized file content", async () => {
            ...
            await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: searchWithCRLF, replace: "Modified Lines" }]) }, { fileContent: contentWithCRLF });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
        it("matches LF search string against CRLF file content after normalization", async () => {
            ...
            await executeSearchAndReplaceTool({ operations: JSON.stringify([{ search: searchWithLF, replace: "Modified Lines" }]) }, { fileContent: contentWithCRLF });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
    });
...
//# sourceMappingURL=searchAndReplaceTool.spec.js.map