...
describe("mode-validator", () => {
    describe("isToolAllowedForMode", () => {
        ...
        describe("dynamic MCP tools", () => {
            ...
            it("allows dynamic MCP tools in custom mode with mcp group", () => {
                ...
                expect(isToolAllowedForMode("mcp_context7_resolve-library-id", "custom-mcp-mode", customModes)).toBe(true);
            });
        });
    ...
    describe("validateToolUse", () => {
        it("throws error for unknown/invalid tools", () => {
            ...
            expect(() => validateToolUse("unknown_tool", "architect", [])).toThrow('Unknown tool "unknown_tool". This tool does not exist.');
        });
        it("throws error for disallowed tools in architect mode", () => {
            // execute_command is a valid tool but not allowed in architect mode
            expect(() => validateToolUse("execute_command", "architect", [])).toThrow('Tool "execute_command" is not allowed in architect mode.');
        });
