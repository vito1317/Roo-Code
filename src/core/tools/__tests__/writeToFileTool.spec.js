...
describe("writeToFileTool", () => {
    ...
    const mockedCreateDirectoriesForFile = createDirectoriesForFile;
    ...
    describe("directory creation for new files", () => {
        it.skipIf(process.platform === "win32")("creates parent directories early when file does not exist (execute)", async () => {
            await executeWriteFileTool({}, { fileExists: false });
            expect(mockedCreateDirectoriesForFile).toHaveBeenCalledWith(absoluteFilePath);
        });
        it.skipIf(process.platform === "win32")("creates parent directories when path has stabilized (partial)", async () => {
