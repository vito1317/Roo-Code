...
describe("searchReplaceTool", () => {
    ...
    async function executeSearchReplaceTool(params = {}, options = {}) {
        ...
        await searchReplaceTool.handle(mockCline, toolUse, {
    ...
    describe("parameter validation", () => {
        ...
        it("allows empty new_string for deletion", async () => {
            ...
            await executeSearchReplaceTool({ old_string: "Line 2", new_string: "" }, { fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("search and replace logic", () => {
        it("returns error when no match is found", async () => {
            const result = await executeSearchReplaceTool({ old_string: "NonExistent" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
        ...
        it("returns error when multiple matches are found", async () => {
            const result = await executeSearchReplaceTool({ old_string: "Line" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
    ...
    describe("CRLF normalization", () => {
        it("normalizes CRLF to LF when reading file", async () => {
            ...
            await executeSearchReplaceTool({ old_string: "Line 2", new_string: "Modified Line 2" }, { fileContent: contentWithCRLF });
            expect(mockCline.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
        it("normalizes CRLF in old_string to match LF-normalized file content", async () => {
            ...
            await executeSearchReplaceTool({ old_string: searchWithCRLF, new_string: "Modified Lines" }, { fileContent: contentWithCRLF });
            expect(mockCline.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
        it("matches LF old_string against CRLF file content after normalization", async () => {
            ...
            await executeSearchReplaceTool({ old_string: searchWithLF, new_string: "Modified Lines" }, { fileContent: contentWithCRLF });
            expect(mockCline.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=searchReplaceTool.spec.js.map