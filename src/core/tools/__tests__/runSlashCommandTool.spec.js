...
describe("runSlashCommandTool", () => {
    ...
    it("should handle command not found", async () => {
        ...
        await runSlashCommandTool.handle(mockTask, block, mockCallbacks);
        expect(mockTask.recordToolError).toHaveBeenCalledWith("run_slash_command");
        expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(formatResponse.toolError("Command 'nonexistent' not found. Available commands: init, test, deploy"));
    });
    ...
    it("should successfully execute built-in command", async () => {
        ...
        vi.mocked(getCommand).mockResolvedValue(mockCommand);
        await runSlashCommandTool.handle(mockTask, block, mockCallbacks);
        expect(mockCallbacks.askApproval).toHaveBeenCalledWith("tool", JSON.stringify({
...
Deploy application to production`);
    ...
    it("should handle partial block", async () => {
        ...
        await runSlashCommandTool.handle(mockTask, block, mockCallbacks);
        expect(mockTask.ask).toHaveBeenCalledWith("tool", JSON.stringify({
            tool: "runSlashCommand",
            command: "init",
            args: "",
        }), true);
    ...
    it("should handle empty available commands list", async () => {
        ...
        await runSlashCommandTool.handle(mockTask, block, mockCallbacks);
        expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(formatResponse.toolError("Command 'nonexistent' not found. Available commands: (none)"));
    });
    ...
    it("should switch mode when mode is specified in command", async () => {
        ...
        expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(`Command: /debug-app
Description: Debug the application
Mode: debug
Source: project
