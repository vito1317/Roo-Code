...
describe("editFileTool", () => {
    ...
    describe("parameter validation", () => {
        ...
        it("treats undefined new_string as empty string (deletion)", async () => {
            await executeEditFileTool({ old_string: "Line 2", new_string: undefined }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockAskApproval).toHaveBeenCalled();
        });
        it("allows empty new_string for deletion", async () => {
            await executeEditFileTool({ old_string: "Line 2", new_string: "" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockAskApproval).toHaveBeenCalled();
        });
        ...
        describe("native tool mode coercion", () => {
            ...
            it("coerces undefined old_string to empty string in native mode (file creation)", async () => {
                await executeWithNativeArgs({ file_path: testFilePath, old_string: undefined, new_string: "New content" }, { fileExists: false });
            ...
            it("coerces undefined new_string to empty string in native mode (deletion)", async () => {
                await executeWithNativeArgs({ file_path: testFilePath, old_string: "Line 2", new_string: undefined }, { fileContent: "Line 1\nLine 2\nLine 3" });
                expect(mockTask.consecutiveMistakeCount).toBe(0);
                expect(mockAskApproval).toHaveBeenCalled();
            });
            it("handles both old_string and new_string as undefined in native mode", async () => {
                await executeWithNativeArgs({ file_path: testFilePath, old_string: undefined, new_string: undefined }, { fileExists: false });
            ...
            it("handles null values as strings in native mode", async () => {
                await executeWithNativeArgs({ file_path: testFilePath, old_string: null, new_string: "New content" }, { fileExists: false });
    ...
    describe("edit_file logic", () => {
        it("returns error when no match is found", async () => {
            const result = await executeEditFileTool({ old_string: "NonExistent" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("No match found");
            expect(result).toContain("<error_details>");
            expect(mockTask.consecutiveMistakeCount).toBe(1);
            expect(mockTask.didToolFailInCurrentTurn).toBe(true);
            expect(mockTask.recordToolError).toHaveBeenCalledWith("edit_file", expect.stringContaining("No match found"));
        });
        ...
        it("returns error when occurrence count does not match expected_replacements", async () => {
            const result = await executeEditFileTool({ old_string: "Line", expected_replacements: "1" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Expected 1 occurrence(s) but found 3");
            expect(result).toContain("<error_details>");
            expect(mockTask.consecutiveMistakeCount).toBe(1);
            expect(mockTask.didToolFailInCurrentTurn).toBe(true);
            expect(mockTask.recordToolError).toHaveBeenCalledWith("edit_file", expect.stringContaining("Occurrence count mismatch"));
        });
        it("succeeds when occurrence count matches expected_replacements", async () => {
            await executeEditFileTool({ old_string: "Line", new_string: "Row", expected_replacements: "4" }, { fileContent: "Line 1\nLine 2\nLine 3\nLine 4" });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
        ...
        it("defaults expected_replacements to 1", async () => {
            const result = await executeEditFileTool({ old_string: "Line" }, { fileContent: "Line 1\nLine 2\nLine 3\nLine 4" });
    ...
    describe("consecutive error display behavior", () => {
        ...
        it("does NOT show diff_error to user on first occurrence_mismatch failure", async () => {
            await executeEditFileTool({ old_string: "Line", expected_replacements: "1" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockTask.consecutiveMistakeCountForEditFile.get(testFilePath)).toBe(1);
            expect(mockTask.say).not.toHaveBeenCalledWith("diff_error", expect.any(String));
            expect(mockTask.recordToolError).toHaveBeenCalledWith("edit_file", expect.stringContaining("Occurrence count mismatch"));
        });
        it("shows diff_error to user on second consecutive occurrence_mismatch failure", async () => {
            ...
            await executeEditFileTool({ old_string: "Line", expected_replacements: "1" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            // Second failure on same file
            await executeEditFileTool({ old_string: "Line", expected_replacements: "5" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockTask.consecutiveMistakeCountForEditFile.get(testFilePath)).toBe(2);
        ...
        it("resets consecutive error counter on successful edit", async () => {
            ...
            expect(mockTask.consecutiveMistakeCountForEditFile.get(testFilePath)).toBe(1);
            // Successful edit
            await executeEditFileTool({ old_string: "Line 2", new_string: "Modified Line 2" }, { fileContent: "Line 1\nLine 2\nLine 3" });
        ...
        it("tracks errors independently per file", async () => {
            ...
            await executeEditFileTool({ file_path: otherFilePath, old_string: "NonExistent" }, { fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("file creation", () => {
        ...
        it("returns error when trying to create file that already exists", async () => {
            const result = await executeEditFileTool({ old_string: "", new_string: "Content" }, { fileExists: true, fileContent: "Existing content" });
    ...
    describe("partial block handling", () => {
        ...
        it("finalizes a partial tool preview row on failure (no stuck spinner)", async () => {
            ...
            await executeEditFileTool({ old_string: "NonExistent" }, { isPartial: true });
            await executeEditFileTool({ old_string: "NonExistent" }, { isPartial: false, fileContent: "Line 1\nLine 2\nLine 3" });
        ...
        it("finalizes a partial tool preview row on no-op success (no changes needed)", async () => {
            // Path stabilization requires two consecutive calls with the same path
            await executeEditFileTool({ old_string: " Line 2", new_string: "Line 2" }, { isPartial: true, fileContent: "Line 1\nLine 2\nLine 3" });
            await executeEditFileTool({ old_string: " Line 2", new_string: "Line 2" }, { isPartial: true, fileContent: "Line 1\nLine 2\nLine 3" });
            const result = await executeEditFileTool({ old_string: " Line 2", new_string: "Line 2" }, { isPartial: false, fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("CRLF normalization", () => {
        it("preserves CRLF line endings on output", async () => {
            ...
            await executeEditFileTool({ old_string: "Line 2", new_string: "Modified Line 2" }, { fileContent: contentWithCRLF });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
    ...
    describe("deterministic fallback matching", () => {
        it("recovers from whitespace/indentation mismatch (whitespace-tolerant regex)", async () => {
            ...
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
            expect(mockTask.diffViewProvider.update).toHaveBeenCalledWith("start\nif (true) {\n    return 2\n}\nend", true);
        });
    ...
    describe("dollar sign handling", () => {
        it("handles $ in new_string correctly", async () => {
            await executeEditFileTool({ old_string: "Line 2", new_string: "Cost: $100" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockAskApproval).toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=editFileTool.spec.js.map