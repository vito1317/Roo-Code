...
vi.mock("../../../integrations/misc/read-file-with-budget", () => ({
    readFileWithTokenBudget: (...args) => mockReadFileWithTokenBudget(...args),
}));
...
vi.mock("../../prompts/responses", () => ({
    formatResponse: {
        ...
        toolDeniedWithFeedback: vi.fn((feedback) => `The user denied this operation and responded with the message:\n<user_message>\n${feedback}\n</user_message>`),
        toolApprovedWithFeedback: vi.fn((feedback) => `The user approved this operation and responded with the message:\n<user_message>\n${feedback}\n</user_message>`),
        rooIgnoreError: vi.fn((path) => `Access to ${path} is blocked by the .rooignore file settings. You must try to continue in the task without using this file, or ask the user to update the .rooignore file.`),
...
beforeEach(() => {
    ...
    addLineNumbersMock.mockImplementation((text, startLine = 1) => {
        if (!text)
            return "";
        const lines = typeof text === "string" ? text.split("\n") : [text];
        return lines.map((line, i) => `${startLine + i} | ${line}`).join("\n");
    ...
    mockReadFileWithTokenBudget.mockImplementation(async (_filePath, _options) => {
...
describe("read_file tool with maxReadFileLine setting", () => {
    ...
    async function executeReadFileTool(params = {}, options = {}) {
        ...
        // Reset the spy before each test
        addLineNumbersMock.mockClear();
        const lineRanges = options.start_line && options.end_line
            ? [
                {
...
describe("read_file tool output structure", () => {
    ...
    async function executeReadFileTool(options = {}) {
        // Configure mocks based on test scenario
        const totalLines = options.totalLines ?? 5;
        const maxReadFileLine = options.maxReadFileLine ?? 500;
...
describe("read_file tool concurrent file reads limit", () => {
    ...
    async function executeReadFileToolWithLimit(fileCount, maxConcurrentFileReads) {
