...
import { providerSettingsWithIdSchema, discriminatedProviderSettingsWithIdSchema, isSecretStateKey, DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, getModelId, isProviderName, } from "@roo-code/types";
...
export class ProviderSettingsManager {
            ...
            return await this.lock(async () => {
                ...
                if (!providerProfiles.migrations.claudeCodeLegacySettingsMigrated) {
                    ...
                    for (const apiConfig of Object.values(providerProfiles.apiConfigs)) {
                        ...
                        if (apiConfig.apiProvider !== "claude-code")
                            continue;
        ...
        try {
            for (const [_name, apiConfig] of Object.entries(providerProfiles.apiConfigs)) {
                ...
                if (newModelId && newModelId !== apiConfig.apiModelId) {
                    console.log(`[ModelMigration] Migrating ${apiConfig.apiProvider} model from ${apiConfig.apiModelId} to ${newModelId}`);
            ...
            return await this.lock(async () => {
                ...
                for (const name in configs) {
                    ...
                    try {
                        ...
                        const supportsReasoningBudget = modelInfo.supportsReasoningBudget || modelInfo.requiredReasoningBudget;
    ...
    sanitizeProviderConfig(apiConfig) {
        ...
        if (config.apiProvider !== undefined && !isProviderName(config.apiProvider)) {
            console.log(`[ProviderSettingsManager] Sanitizing invalid provider "${config.apiProvider}" - resetting to undefined`);
