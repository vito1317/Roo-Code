...
// --- Render functions ---
function renderButton(args: any): string {
...
function renderProgress(args: any): string {
...
function renderAlert(args: any): string {
...
function renderCodeBlock(args: any): string {
  const { code, language, title, showLineNumbers = true } = args;
  const lines = code.split("\n");
  const codeHtml = showLineNumbers
    ? lines.map((line: string, i: number) => `<span style="color:#6b7280;margin-right:16px;">${i + 1}</span>${escapeHtml(line)}`).join("\n")
    : escapeHtml(code);
...
function renderDivider(args: any): string {
...
function renderStats(args: any): string {
...
function renderMermaid(args: any): string {
