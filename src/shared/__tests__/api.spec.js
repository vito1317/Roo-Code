...
describe("shouldUseReasoningEffort", () => {
    ...
    test("boolean true with 'none' and 'minimal' -> true", () => {
        ...
        expect(shouldUseReasoningEffort({ model, settings: { reasoningEffort: "none" } })).toBe(true);
        expect(shouldUseReasoningEffort({ model, settings: { reasoningEffort: "minimal" } })).toBe(true);
    });
});
