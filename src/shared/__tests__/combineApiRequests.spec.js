...
describe("combineApiRequests", () => {
    ...
    describe("Edge cases", () => {
        ...
        it("should properly merge nested JSON objects", () => {
            const startMessage = createStartMessage('{"request":"GET /api/data", "metadata": {"source": "user", "priority": "high"}}', 1000);
            const finishMessage = createFinishMessage('{"cost":0.005, "metadata": {"duration": 200, "priority": "override"}}', 1001);
            const messages = [startMessage, finishMessage];
            const result = combineApiRequests(messages);
            // Should have one message (the combined one)
            expect(result).toHaveLength(1);
        ...
        it("should handle complex JSON objects with multiple properties", () => {
            const startMessage = createStartMessage('{"request":"GET /api/data", "user": "john", "timestamp": 1616161616, "params": {"page": 1, "limit": 10}}', 1000);
            const finishMessage = createFinishMessage('{"cost":0.005, "duration": 150, "cache": false, "results": {"count": 42, "status": "success"}}', 1001);
            const messages = [startMessage, finishMessage];
            const result = combineApiRequests(messages);
            // Should have one message (the combined one)
            expect(result).toHaveLength(1);
...
//# sourceMappingURL=combineApiRequests.spec.js.map