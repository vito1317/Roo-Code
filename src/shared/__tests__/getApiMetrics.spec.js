...
describe("getApiMetrics", () => {
    ...
    const createCondenseContextMessage = (cost = 0.002, newContextTokens = 500, prevContextTokens = 1000, ts = 2000) => ({
        type: "say",
    ...
    describe("Basic functionality", () => {
        it("should calculate metrics from a single api_req_started message", () => {
            const messages = [
                createApiReqStartedMessage('{"tokensIn":100,"tokensOut":200,"cacheWrites":5,"cacheReads":10,"cost":0.005}'),
            ];
        ...
        it("should calculate metrics from multiple api_req_started messages", () => {
            const messages = [
                createApiReqStartedMessage('{"tokensIn":100,"tokensOut":200,"cacheWrites":5,"cacheReads":10,"cost":0.005}', 1000),
                createApiReqStartedMessage('{"tokensIn":50,"tokensOut":150,"cacheWrites":3,"cacheReads":7,"cost":0.003}', 2000),
            ];
            const result = getApiMetrics(messages);
        ...
        it("should calculate metrics from mixed message types", () => {
            const messages = [
                ...
                createApiReqStartedMessage('{"tokensIn":50,"tokensOut":150,"cacheWrites":3,"cacheReads":7,"cost":0.003}', 3000),
            ];
            const result = getApiMetrics(messages);
            expect(result.totalTokensIn).toBe(150); // 100 + 50
            expect(result.totalTokensOut).toBe(350); // 200 + 150
    ...
    describe("Edge cases", () => {
        ...
        it("should handle non-number values in api_req_started message", () => {
            const messages = [
                ...
                createApiReqStartedMessage('{"tokensIn":"not-a-number","tokensOut":"not-a-number","cacheWrites":"not-a-number","cacheReads":"not-a-number","cost":"not-a-number"}'),
            ];
            const result = getApiMetrics(messages);
...
//# sourceMappingURL=getApiMetrics.spec.js.map