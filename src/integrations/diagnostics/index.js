...
export async function diagnosticsToProblemsString(diagnostics, severities, cwd, includeDiagnosticMessages = true, maxDiagnosticMessages) {
    ...
    if (maxDiagnosticMessages && maxDiagnosticMessages > 0) {
        ...
        for (const { uri, diagnostics: fileDiagnostics } of groupedDiagnostics.values()) {
            const sortedDiagnostics = fileDiagnostics.sort((a, b) => a.diagnostic.range.start.line - b.diagnostic.range.start.line);
