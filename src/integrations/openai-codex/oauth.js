...
function parseOAuthErrorDetails(errorText) {
    try {
        ...
        const errorMessage = typeof errorDescription === "string"
            ? errorDescription
...
export async function refreshAccessToken(credentials) {
    ...
    if (!response.ok) {
        ...
        throw new OpenAiCodexOAuthTokenError(`Token refresh failed: ${response.status} ${response.statusText}${details ? ` - ${details}` : ""}`, { status: response.status, errorCode });
    }
    const data = await response.json();
    const tokenResponse = tokenResponseSchema.parse(data);
    // Per the implementation guide: expires is in milliseconds since epoch
        ...
        try {
            ...
            if (!this.refreshPromise) {
                ...
                this.refreshPromise = refreshAccessToken(this.credentials).then((newCreds) => {
                    ...
                    this.log(`[openai-codex-oauth] Forced refresh response received (expires_in≈${Math.round((newCreds.expires - Date.now()) / 1000)}s, refresh_token_rotated=${rotated})`);
                    return newCreds;
                });
            }
    ...
    async getAccessToken() {
        ...
        if (isTokenExpired(this.credentials)) {
            try {
                // De-dupe concurrent refreshes
                if (!this.refreshPromise) {
                    this.log(`[openai-codex-oauth] Access token expired (expires=${this.credentials.expires}). Refreshing...`);
                    const prevRefreshToken = this.credentials.refresh_token;
                    this.refreshPromise = refreshAccessToken(this.credentials).then((newCreds) => {
                        const rotated = newCreds.refresh_token !== prevRefreshToken;
                        this.log(`[openai-codex-oauth] Refresh response received (expires_in≈${Math.round((newCreds.expires - Date.now()) / 1000)}s, refresh_token_rotated=${rotated})`);
                        return newCreds;
                    });
                }
                const newCredentials = await this.refreshPromise;
...
<script>setTimeout(() => window.close(), 3000);</script>
            ...
            server.on("error", (err) => {
                ...
                if (err.code === "EADDRINUSE") {
                    reject(new Error(`Port ${OPENAI_CODEX_OAUTH_CONFIG.callbackPort} is already in use. ` +
