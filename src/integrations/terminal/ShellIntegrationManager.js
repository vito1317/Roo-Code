...
export class ShellIntegrationManager {
    ...
    static zshCleanupTmpDir(terminalId) {
        ...
        try {
            ...
            console.error(`[TerminalRegistry] Error cleaning up temporary directory ${tmpDir}: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    static clear() {
    ...
    static getShellIntegrationPath(shell) {
        ...
        return path.join(vscode.env.appRoot, "out", "vs", "workbench", "contrib", "terminal", "common", "scripts", filename);
    }
}
//# sourceMappingURL=ShellIntegrationManager.js.map