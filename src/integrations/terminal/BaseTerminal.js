...
export class BaseTerminal {
    ...
    setActiveStream(stream, pid) {
        if (stream) {
            if (!this.process) {
                this.running = false;
                console.warn(`[Terminal ${this.provider}/${this.id}] process is undefined, so cannot set terminal stream (probably user-initiated non-Roo command)`);
                return;
            }
            this.running = true;
            this.streamClosed = false;
     ...
     * @returns The compressed terminal output
     */
    static compressTerminalOutput(input, lineLimit, characterLimit) {
