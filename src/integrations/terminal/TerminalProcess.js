...
export class TerminalProcess extends BaseTerminalProcess {
    ...
    async run(command) {
        ...
        if (!isShellIntegrationAvailable) {
            ...
            console.warn("[TerminalProcess] Shell integration not available. Command sent without knowledge of response.");
            this.emit("no_shell_integration", "Command was submitted; output is not available, as shell integration is inactive.");
            this.emit("completed", "<shell integration is not available, so terminal output and command execution status is unknown>");
            this.emit("continue");
            return;
        }
        ...
        const streamAvailable = new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                ...
                this.emit("no_shell_integration", `VSCE shell integration stream did not start within ${Terminal.getShellIntegrationTimeout() / 1000} seconds. Terminal problem?`);
                // Reject with descriptive error
                reject(new Error(`VSCE shell integration stream did not start within ${Terminal.getShellIntegrationTimeout() / 1000} seconds.`));
        ...
        try {
            ...
            this.emit("completed", "<VSCE shell integration stream did not start: terminal output and command execution status is unknown>");
            this.terminal.busy = false;
        ...
        if (commandOutputStarted) {
            ...
            const errorMsg = "VSCE output start escape sequence (]633;C or ]133;C) not received, but the stream has started. Upstream VSCE Bug?";
            ...
            // Emit completed event with error message
            this.emit("completed", "<VSCE shell integration markers not found: terminal output and command execution status is unknown>\n" +
