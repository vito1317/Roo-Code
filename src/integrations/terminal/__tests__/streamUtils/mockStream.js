 ...
 */
export function createBaseMockStream(output, exitCode = 0) {
    const stream = {
 ...
 * @returns An object containing the stream and exit code
 */
export function createBashMockStream(output, exitCode = 0) {
 ...
 * @returns An object containing the stream and exit code
 */
export function createCmdMockStream(output, exitCode = 0) {
 ...
 * @returns An object containing the stream and exit code
 */
export function createPowerShellMockStream(output, exitCode = 0) {
 ...
 * @returns An object containing the stream and exit code
 */
export function createChunkedMockStream(output, chunkSize = 100, exitCode = 0) {
    const stream = {
        async *[Symbol.asyncIterator]() {
            // Start marker
            yield "\x1b]633;C\x07";
