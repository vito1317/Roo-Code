 ...
 */
export function getSupportedBinaryFormats() {
 ...
 */
export async function extractTextFromFile(filePath, maxReadFileLine) {
    ...
    if (maxReadFileLine !== undefined && maxReadFileLine !== -1) {
        if (!Number.isInteger(maxReadFileLine) || maxReadFileLine < 1) {
            throw new Error(`Invalid maxReadFileLine: ${maxReadFileLine}. Must be a positive integer or -1 for unlimited.`);
        }
    }
    try {
 ...
 */
export function stripLineNumbers(content, aggressive = false) {
 ...
 */
export function truncateOutput(content, lineLimit, characterLimit) {
 ...
 */
export function applyRunLengthEncoding(content) {
    if (!content) {
        return content;
    }
    let result = "";
 ...
 */
export function processCarriageReturns(input) {
 ...
 */
export function processBackspaces(input) {
    let output = "";
    let pos = 0;
    ...
    while (crPos < lineEnd) {
        ...
        if (segment !== "") {
            ...
            if (segment.length >= curLine.length) {
                ...
                if ((segmentLastCharCode >= 0xd800 && segmentLastCharCode <= 0xdbff) || // High surrogate at end of segment
