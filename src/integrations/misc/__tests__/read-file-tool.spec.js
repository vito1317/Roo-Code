...
describe("read_file tool with maxReadFileLine setting", () => {
    ...
    let originalCountFileLines;
    let originalReadLines;
    let originalExtractTextFromFile;
    let originalAddLineNumbers;
    beforeEach(async () => {
        // Import actual implementations
        originalCountFileLines = (await vitest.importActual("../line-counter")).countFileLines;
        originalReadLines = (await vitest.importActual("../read-lines")).readLines;
        originalExtractTextFromFile = (await vitest.importActual("../extract-text")).extractTextFromFile;
        originalAddLineNumbers = (await vitest.importActual("../extract-text")).addLineNumbers;
        vitest.resetAllMocks();
        countFileLines.mockImplementation(originalCountFileLines);
        readLines.mockImplementation(originalReadLines);
        extractTextFromFile.mockImplementation(originalExtractTextFromFile);
        addLineNumbers.mockImplementation(originalAddLineNumbers);
    });
    ...
    it("should read entire file when line count is less than maxReadFileLine", async () => {
        ...
        ;
        countFileLines.mockResolvedValue(100);
        extractTextFromFile.mockResolvedValue("Small file content");
    ...
    it("should truncate file when line count exceeds maxReadFileLine", async () => {
        // Mock necessary functions
        ;
        countFileLines.mockResolvedValue(5000);
        readLines.mockResolvedValue("First 500 lines of large file");
        addLineNumbers.mockReturnValue("1 | First line\n2 | Second line\n...");
    ...
    it("should add source code file type info for large source code files", async () => {
        ...
        ;
        countFileLines.mockResolvedValue(5000);
        readLines.mockResolvedValue("First 500 lines of large JavaScript file");
        addLineNumbers.mockReturnValue('1 | const foo = "bar";\n2 | function test() {...');
...
//# sourceMappingURL=read-file-tool.spec.js.map