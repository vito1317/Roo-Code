...
describe("extractTextFromFile - Large File Handling", () => {
    ...
    it("should truncate files that exceed maxReadFileLine limit", async () => {
        ...
        mockedReadLines.mockResolvedValue(Array(100)
            .fill(null)
            .map((_, i) => `Line ${i + 1}: This is a test line with some content`)
            .join("\n"));
        ...
        expect(result).toContain("[File truncated: showing 100 of 150 total lines. The file is too large and may exhaust the context window if read in full.]");
    });
    it("should not truncate files within the maxReadFileLine limit", async () => {
        ...
        mockedFs.readFile.mockResolvedValue(smallFileContent);
    ...
    it("should handle files with exactly maxReadFileLine lines", async () => {
        ...
        mockedFs.readFile.mockResolvedValue(exactFileContent);
    ...
    it("should handle undefined maxReadFileLine by not truncating", async () => {
        const largeFileContent = Array(200)
            ...
            .map((_, i) => `Line ${i + 1}`)
            .join("\n");
        mockedFs.readFile.mockResolvedValue(largeFileContent);
    ...
    it("should handle very large files efficiently", async () => {
        ...
        mockedReadLines.mockResolvedValue(Array(500)
            .fill(null)
    ...
    it("should handle maxReadFileLine of 0 by throwing an error", async () => {
        ...
        await expect(extractTextFromFile("/test/file.ts", 0)).rejects.toThrow("Invalid maxReadFileLine: 0. Must be a positive integer or -1 for unlimited.");
    });
    ...
    it("should handle binary files by throwing an error", async () => {
        ...
        await expect(extractTextFromFile("/test/binary.bin", 100)).rejects.toThrow("Cannot read text for file type: .bin");
    });
    it("should handle file not found errors", async () => {
        mockedFs.access.mockRejectedValue(new Error("ENOENT"));
        await expect(extractTextFromFile("/test/nonexistent.ts", 100)).rejects.toThrow("File not found: /test/nonexistent.ts");
    });
});
//# sourceMappingURL=extract-text-large-files.spec.js.map