...
function runPerformanceTest(name, fn, input, iterations, args = []) {
    ...
    console.log(`- Output size: ${resultSize} MB (${reduction}% ${parseFloat(reduction) < 0 ? "increase" : "reduction"})`);
    return {
        stats,
        resultSize,
        reduction,
...
function runBaselineTest(input, iterations) {
    ...
    const throughputVariation = Math.max(...throughputs.map((t) => Math.abs(((t - avgThroughput) / avgThroughput) * 100)));
...
function runBenchmark() {
    ...
    for (const [size, complexity] of standardTestConfigs) {
        ...
        console.log("\n--- Combined Pipeline ---");
        runPerformanceTest("Full Pipeline", (input) => truncateOutput(applyRunLengthEncoding(processCarriageReturns(input)), lineLimit), testData, Math.max(3, Math.floor(iterations / 5)));
    }
    // Run long line tests
    console.log("\n" + "=".repeat(80));
...
//# sourceMappingURL=processCarriageReturns.benchmark.js.map