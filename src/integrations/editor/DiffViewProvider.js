...
export class DiffViewProvider {
    ...
    async open(relPath) {
        ...
        // contents.
        if (fileExists) {
            const existingDocument = vscode.workspace.textDocuments.find((doc) => doc.uri.scheme === "file" && arePathsEqual(doc.uri.fsPath, absolutePath));
    ...
    async saveChanges(diagnosticsEnabled = true, writeDelayMs = DEFAULT_WRITE_DELAY_MS) {
        ...
        return new Promise((resolve, reject) => {
            ...
            disposables.push(vscode.workspace.onDidOpenTextDocument(async (document) => {
                ...
                if (document.uri.scheme === "file" && arePathsEqual(document.uri.fsPath, uri.fsPath)) {
                    ...
                    const editor = vscode.window.visibleTextEditors.find((e) => e.document.uri.scheme === "file" && arePathsEqual(e.document.uri.fsPath, uri.fsPath));
                    if (editor) {
                        cleanup();
                        resolve(editor);
                    }
            ...
            vscode.window
                ...
                return vscode.commands.executeCommand("vscode.diff", vscode.Uri.parse(`${DIFF_VIEW_URI_SCHEME}:${fileName}`).with({
                    query: Buffer.from(this.originalContent ?? "").toString("base64"),
                }), uri, `${fileName}: ${fileExists ? `${DIFF_VIEW_LABEL_CHANGES}` : "New File"} (Editable)`, { preserveFocus: true });
            })
                .then(() => {
     ...
     */
    async saveDirectly(relPath, content, openFile = true, diagnosticsEnabled = true, writeDelayMs = DEFAULT_WRITE_DELAY_MS) {
        const absolutePath = path.resolve(this.cwd, relPath);
        ...
        if (diagnosticsEnabled) {
            ...
            const includeDiagnosticMessages = state?.includeDiagnosticMessages ?? true;
            const maxDiagnosticMessages = state?.maxDiagnosticMessages ?? 50;
            const newProblems = await diagnosticsToProblemsString(getNewDiagnostics(this.preDiagnostics, postDiagnostics), [vscode.DiagnosticSeverity.Error], this.cwd, includeDiagnosticMessages, maxDiagnosticMessages);
            newProblemsMessage =
                newProblems.length > 0 ? `\n\nNew problems detected after saving the file:\n${newProblems}` : "";
        }
